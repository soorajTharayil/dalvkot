...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateTimeFormat = function () {
      // Build the total time string (your original code)
      ...
      // Read raw input values (for empty check)
      ...
      var denVal = document.getElementById("formula_para2").value;

      // Alert ONLY if fields are EMPTY (not zero)
      if (hrVal === "" || minVal === "" || secVal === "") {
        alert("Please enter hours, minutes, and seconds.");
        return;
      }
      ...
      // Convert to numbers
      var hoursInput = parseInt(hrVal) || 0;
      var minutesInput = parseInt(minVal) || 0;
      var secondsInput = parseInt(secVal) || 0;

      // Convert into total seconds
      var assessmentSeconds =
        hoursInput * 3600 + minutesInput * 60 + secondsInput;

      var totalBloodCrossmatched = parseInt(denVal) || 0;

      // If both numerator and denominator are zero → result is 0:00:00
      if (assessmentSeconds === 0 && totalBloodCrossmatched === 0) {
      ...
      // If denominator is zero but numerator is not → result = 0
      if (totalBloodCrossmatched === 0) {
        $scope.calculatedResult = "0:00:00";
        $scope.feedback.calculatedResult = $scope.calculatedResult;
        $scope.valuesEdited = false;
        return;
