...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window) {
	...
	$scope.setupapplication = function () {
		...
		//alert(id);
		$http.get($rootScope.baseurl_main + '/api_14PSQ3b.php?patientid=' + id + '&month=' + selectedMonths + '&year=' + selectedYears, { timeout: 20000 }).then(function (responsedata) {
	...
	$scope.calculateVAPRate = function () {
		...
		var ventilatorAssociatedPneumonia = parseInt(document.getElementById('formula_para1').value);

		// Get the number of ventilator days in that month
		var ventilatorDays = parseInt(document.getElementById('formula_para2').value);
		...
		if (ventilatorAssociatedPneumonia === 0 && ventilatorDays === 0) {
			...
			$scope.calculatedResult = Math.round((ventilatorAssociatedPneumonia / ventilatorDays) * 1000);
		}
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_14PSQ3b')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/savepatientfeedback_kpi14.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
