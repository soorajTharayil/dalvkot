...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateVAPRate = function () {
      // Get raw values
      var numVal = document.getElementById("formula_para1").value;
      var denVal = document.getElementById("formula_para2").value;

      // Check for EMPTY values
      if (numVal === "" || numVal === null) {
        alert(
          "Enter the number of ventilator associated pneumonia in a month."
        );
        return;
      }

      if (denVal === "" || denVal === null) {
        alert("Enter the number of ventilator days in that month.");
        return;
      }

      // Safe parse (now inputs are guaranteed non-empty)
      var ventilatorAssociatedPneumonia = parseInt(numVal);
      var ventilatorDays = parseInt(denVal);
