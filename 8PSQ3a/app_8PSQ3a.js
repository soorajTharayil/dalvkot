...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window) {
	...
	$scope.setupapplication = function () {
		...
		//alert(id);
		$http.get($rootScope.baseurl_main + '/api_8PSQ3a.php?patientid=' + id, { timeout: 20000 }).then(function (responsedata) {
	...
	$scope.calculateAdherenceToProcedureRate = function () {
		...
		var surgeriesWithProcedureFollowed = parseInt(document.getElementById('formula_para1').value);

		// Get the total number of surgeries performed
		var totalSurgeriesPerformed = parseInt(document.getElementById('formula_para2').value);
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_8PSQ3a')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/savepatientfeedback_kpi8.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
