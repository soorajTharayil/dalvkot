...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateAdherenceToProcedureRate = function () {
      ...
      // Validate inputs (allow zero for denominator, only block NaN)
      if (
      ...
      if (isNaN(totalSurgeriesPerformed)) {
        alert("Enter the total number of surgeries performed.");
        return;
      }

      // Skip comparison if total surgeries = 0
      if (
        totalSurgeriesPerformed > 0 &&
        surgeriesWithProcedureFollowed > totalSurgeriesPerformed
      ) {
        alert(
      ...
      if (totalSurgeriesPerformed === 0) {
        adherenceToProcedureRate = 0; // avoid division by zero
      } else {
        adherenceToProcedureRate =
