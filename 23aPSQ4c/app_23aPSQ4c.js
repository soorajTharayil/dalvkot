...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window) {
	...
	$scope.setupapplication = function () {
		...
		//alert(id);
		$http.get($rootScope.baseurl_main + '/api_23aPSQ4c.php?patientid=' + id + '&month=' + selectedMonths + '&year=' + selectedYears, { timeout: 20000 }).then(function (responsedata) {
			$scope.feedback.initial_assessment_hr = responsedata.data.initial_assessment_hr;
			$scope.feedback.initial_assessment_min = responsedata.data.initial_assessment_min;
			$scope.feedback.initial_assessment_sec = responsedata.data.initial_assessment_sec;
	...
	$scope.calculateTimeFormat = function () {
		...
		// Get total admission
		var totalPatientInDiagnostics = parseInt(document.getElementById('formula_para2').value);
		...
		var averageSeconds = Math.floor(assessmentSeconds / totalPatientInDiagnostics);
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_23aPSQ4c')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/savepatientfeedback_kpi23a.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
