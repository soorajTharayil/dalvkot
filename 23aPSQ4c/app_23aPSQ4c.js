...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateTimeFormat = function () {
      ...
      // ALERT if numerator EMPTY (not zero)
      if (hoursInput === "" || minutesInput === "" || secondsInput === "") {
        alert("Please enter all time fields (hr, min, sec)");
        return;
      }

      hoursInput = parseInt(hoursInput) || 0;
      ...
      // ALERT if denominator EMPTY (not zero)
      ...
      if (assessmentSeconds === 0 && totalPatientInDiagnostics === 0) {
      ...
      // Case 2 → numerator > 0 and denominator = 0 → return numerator time itself
      if (totalPatientInDiagnostics === 0) {
        var h = Math.floor(assessmentSeconds / 3600);
        var rem = assessmentSeconds % 3600;

        var m = Math.floor(rem / 60);
        var s = rem % 60;

        $scope.calculatedResult = `${h}:${("0" + m).slice(-2)}:${(
          "0" + s
        ).slice(-2)}`;
        $scope.feedback.calculatedResult = $scope.calculatedResult;
        $scope.valuesEdited = false;
