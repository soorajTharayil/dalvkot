...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateErrorRate = function () {
      ...
      // Validate reporting errors (only check NaN, allow 0)
      if (isNaN(reportingErrors) || reportingErrors < 0) {
        alert("Please enter the number of reporting errors.");
        return;
      }

      // Validate testsPerformed (only check NaN, allow 0)
      if (isNaN(testsPerformed)) {
        alert("Please enter the number of tests performed.");
        return;
      }

      // reportingErrors > testsPerformed (skip check if testsPerformed is 0)
      ...
      // Calculate errors per 1000 — handle denominator = 0 safely
      var errorsPerThousand;

      if (testsPerformed === 0) {
        errorsPerThousand = 0; // Avoid division by zero
      } else {
