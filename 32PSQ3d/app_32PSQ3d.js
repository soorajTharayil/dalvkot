...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window) {
	...
	$scope.setupapplication = function () {
		...
		//alert(id);
		$http.get($rootScope.baseurl_main + '/api_32PSQ3d.php?patientid=' + id + '&month=' + selectedMonths + '&year=' + selectedYears, { timeout: 20000 }).then(function (responsedata) {
			$scope.feedback.initial_assessment_hr = responsedata.data.capitalCount;
			$scope.feedback.total_admission = responsedata.data.prescriptionsCount;
	...
	$scope.calculateComplianceRate = function () {
		...
		var prescriptionsInCapitals = parseInt(document.getElementById('formula_para1').value);

		// Get the total number of prescriptions
		var totalPrescriptions = parseInt(document.getElementById('formula_para2').value);
		...
		if (prescriptionsInCapitals > totalPrescriptions) {
			alert("The number of prescriptions in capital letters must be less than the total number of prescriptions.");
			return;
		}

		// Calculate the percentage of compliance rate to medication prescription in capitals
		$scope.calculatedResult = ((prescriptionsInCapitals / totalPrescriptions) * 100).toFixed(2);
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_32PSQ3d')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/savepatientfeedback_kpi32.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
