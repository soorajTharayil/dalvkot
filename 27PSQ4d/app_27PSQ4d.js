...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window) {
	...
	$scope.setupapplication = function () {
		...
		//alert(id);
		$http.get($rootScope.baseurl_main + '/api_27PSQ4d.php?patientid=' + id + '&month=' + selectedMonths + '&year=' + selectedYears, { timeout: 20000 }).then(function (responsedata) {
	...
	$scope.calculateMockDrillVariationsRate = function () {
		...
		var variationsObserved = parseInt(document.getElementById('formula_para1').value);

		// Get the total number of opportunities of variations
		var opportunitiesOfVariations = parseInt(document.getElementById('formula_para2').value);
		...
		if (variationsObserved === 0 && opportunitiesOfVariations === 0) {
			...
			$scope.calculatedResult = ((variationsObserved / opportunitiesOfVariations) * 100).toFixed(2);
		}

		// Store the result in the feedback object for further use
		$scope.feedback.calculatedResult = $scope.calculatedResult;

		console.log("Calculated Mock Drill Variations Rate", $scope.calculatedResult);
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_27PSQ4d')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/savepatientfeedback_kpi27.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
