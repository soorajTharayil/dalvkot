...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window) {
	...
	$scope.setupapplication = function () {
		...
		//alert(id);
		$http.get($rootScope.baseurl_main + '/api_16PSQ3b.php?patientid=' + id + '&month=' + selectedMonths + '&year=' + selectedYears, { timeout: 20000 }).then(function (responsedata) {
	...
	$scope.calculateSSIRate = function () {
		...
		var surgicalSiteInfections = parseInt(document.getElementById('formula_para1').value);

		// Get the number of surgeries performed in that month
		var surgeriesPerformed = parseInt(document.getElementById('formula_para2').value);
		...
		if (surgicalSiteInfections === 0 && surgeriesPerformed === 0) {
			...
			$scope.calculatedResult = ((surgicalSiteInfections / surgeriesPerformed) * 100).toFixed(2);
		}
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_16PSQ3b')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/savepatientfeedback_kpi16.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
