...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window) {
	...
	$scope.setupapplication = function () {
		...
		// alert(id);
		$http.get($rootScope.baseurl_main + '/api_31PSQ3d.php?patientid=' + id + '&month=' + selectedMonths + '&year=' + selectedYears, { timeout: 20000 }).then(function (responsedata) {
			console.log(responsedata.data);
			console.log("hi");
			$scope.feedback.initial_assessment_hr = responsedata.data.handoverCount;
	...
	$scope.calculateHandoversRate = function () {
		...
		var appropriateHandovers = parseInt(document.getElementById('formula_para1').value);
		...
		if (appropriateHandovers === 0 && handoverOpportunities === 0) {
			...
			$scope.calculatedResult = ((appropriateHandovers / handoverOpportunities) * 100).toFixed(2);
		}
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_31PSQ3d')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/savepatientfeedback_kpi31.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
