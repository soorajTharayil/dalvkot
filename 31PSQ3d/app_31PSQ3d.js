...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateHandoversRate = function () {
      ...
      var appropriateHandovers = document.getElementById("formula_para1").value;

      // Get the total number of handover opportunities
      var handoverOpportunities =
        document.getElementById("formula_para2").value;

      // Alert if inputs are empty
      if (appropriateHandovers === "") {
        alert("Enter the total number of handovers done appropriately.");
        return;
      }
      if (handoverOpportunities === "") {
        alert("Enter the total number of handover opportunities.");
        return;
      }

      // Convert to integers
      appropriateHandovers = parseInt(appropriateHandovers);
      handoverOpportunities = parseInt(handoverOpportunities);

      // Validate negative numbers
      if (appropriateHandovers < 0) {
        alert("Enter the total number of handovers done appropriately.");
        return;
      }
      ...
      if (appropriateHandovers === 0 && handoverOpportunities === 0) {
        ...
        // Calculate the percentage of appropriate handovers safely
        $scope.calculatedResult = (
          (appropriateHandovers / (handoverOpportunities || 1)) *
          100
        ).toFixed(2);
      }
