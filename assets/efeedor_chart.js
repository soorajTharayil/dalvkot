...
function patient_feedback_analysis(callback) {
  ...
  var apiUrl = "http://" + domain + "/analytics/patient_feedback_analysis"; // Replace with your API endpoint
    ...
    if (downloadBtn) {
      downloadBtn.onclick = function (e) {
        ...
        if (canvas) {
          ...
          link.href = url;
          link.download = 'patient_feedback_' + new Date().toISOString().slice(0, 10) + '.png';
...
function handleBarClick(clickData) {
  ...
  var url = "http://" + domain + "/ipd/trend_analytic/" + clickData.type;
  window.location = url;
...
function patient_satisfaction_analysis(callback) {
  var xhr = new XMLHttpRequest();
  var apiUrl = "http://" + domain + "/analytics/patient_satisfaction_analysis"; // Replace with your API endpoint
  xhr.open("GET", apiUrl, true);
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4 && xhr.status === 200) {
      var responseData = JSON.parse(xhr.responseText);
...
function net_permoter_analysis(callback) {
  var xhr = new XMLHttpRequest();
  var apiUrl = "http://" + domain + "/analytics/net_permoter_analysis"; // Replace with your API endpoint
  xhr.open("GET", apiUrl, true);
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4 && xhr.status === 200) {
      var responseData = JSON.parse(xhr.responseText);
...
function why_patient_choose(callback) {
  var xhr = new XMLHttpRequest();
  var apiUrl = "http://" + domain + "/analytics/why_patient_choose"; // Replace with your API endpoint
  xhr.open("GET", apiUrl, true);
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4 && xhr.status === 200) {
      var responseData = JSON.parse(xhr.responseText);
...
function tickets_recived_by_department(callback) {
  var xhr = new XMLHttpRequest();
  var apiUrl = "http://" + domain + "/analytics/tickets_recived_by_department"; // Replace with your API endpoint
  xhr.open("GET", apiUrl, true);
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4 && xhr.status === 200) {
      var responseData = JSON.parse(xhr.responseText);
...
function resposnsechart(callback) {
  var xhr = new XMLHttpRequest();
  var apiUrl = "http://" + domain + "/analytics/resposnsechart"; // Replace with your API endpoint
  xhr.open("GET", apiUrl, true);
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4 && xhr.status === 200) {
      var responseData = JSON.parse(xhr.responseText);
...
function handleRequest(url, type) {
  ...
  var xhr = new XMLHttpRequest();
  xhr.open("POST", "http://" + domain + "/process_image.php", true);
  xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
  xhr.onreadystatechange = function () {
    if (this.readyState === XMLHttpRequest.DONE && this.status === 200) {
      console.log("Response from server: " + this.responseText);
