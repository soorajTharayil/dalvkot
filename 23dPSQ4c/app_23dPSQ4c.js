...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateTimeFormat = function () {
      ...
      // ✔ Allow zero, only block EMPTY
      if (
        totalPatientInDiagnostics === "" ||
        totalPatientInDiagnostics === null
      ) {
        alert("Please enter the above value to calculate");
        return;
      }

      totalPatientInDiagnostics = parseInt(totalPatientInDiagnostics);

      // ✔ If denominator = 0 → return result as 0:00:00 instead of error
      var averageSeconds =
        totalPatientInDiagnostics === 0
          ? 0
          : Math.floor(assessmentSeconds / totalPatientInDiagnostics);
