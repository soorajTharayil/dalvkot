...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateReturnToEmergencyKPI = function () {
      ...
      // Validate inputs for returns to emergency and total patients (allow zero)
      ...
      // Only validate greater-than condition if denominator > 0
      if (totalPatients > 0 && returnsToEmergency > totalPatients) {
        alert(
      ...
      // Safe handling for denominator zero → result = 0
      var returnToEmergencyKPI = 0;

      if (totalPatients !== 0) {
        returnToEmergencyKPI = (returnsToEmergency / totalPatients) * 100;
      }

      // Format the result: whole number or 2 decimals
