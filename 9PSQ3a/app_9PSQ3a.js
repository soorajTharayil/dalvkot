...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateTransfusionReactionRate = function () {
      // Get the number of transfusion reactions (numerator)
      var transfusionReactions = parseFloat(
        document.getElementById("formula_para1").value
      );

      // Get the total number of units transfused (denominator)
      var totalUnitsTransfused = parseFloat(
        document.getElementById("formula_para2").value
      );

      // Validate numerator (allow zero)
      if (isNaN(transfusionReactions) || transfusionReactions < 0) {
        alert("Enter the number of transfusion reactions.");
        return;
      }

      // Validate denominator (allow zero also)
      ...
      if (
        ...
        transfusionReactions > totalUnitsTransfused
      ) {
        alert("Transfusion reactions cannot exceed total units transfused.");
        return;
      }

      // If denominator is zero → result should be zero instead of dividing
      var transfusionReactionRate = 0;

      if (totalUnitsTransfused !== 0) {
        transfusionReactionRate =
