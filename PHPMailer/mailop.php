...
include('../api/db.php');
$mail->Subject = 'HID: '.$HID.' NEW OUTPATIENT FEEDBACK TICKET- EFEEDOR';
...
while($r = mysqli_fetch_object($flore)){
	...
	while($t = mysqli_fetch_object($ticket)){
		...
		$message .= 'Feedback ticket from <br />Patient: '.$p->name.', <br />PID: '.$p->patient_id.', <br />Department: '.$t->description;
		$section = '';
	...
	if($rowcount > 1){
			...
			$messages .= 'Feedback ticket from <br />Patient:  '.$p->name.',<br /> PID: '.$p->patient_id.','; 
	...
	if($rowcount > 0){
		...
		while($u = mysqli_fetch_object($patient)){
			...
			if($permission->email_ticket_op == 1){
				$mail->ClearAddresses();
				$mail->addAddress($u->email, 'Info ABC');
