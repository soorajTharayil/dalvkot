			...
			<div class="col-lg-12 col-sm-12">
				<div class="panel panel-default">
					...
					<div class="panel-body" style="font-size:14px; line-height:1.5;">
						...
						$auditTitle = 'Laboratory safety adherence audit';
						...
						if (!empty($users)) {
							foreach ($users as $user) {
								...
								foreach ($perms as $p) {
									if (strcasecmp(trim($p->feature_name), 'AUDIT-FORM2') === 0) {
						...
						<table class="table table-bordered table-condensed" style="margin:0;">
							<tbody>
								<tr>
									...
									<td>Ensures lab staff follow PPE, hygiene, biomedical waste, and biosafety protocols per NABH, WHO, ICMR, CDC, CAHO & JCI standards to ensure safety, compliance, and infection control.</td>
								</tr>
								<tr>
			...
			<div class="col-lg-12">
				<div class="panel panel-default">
					<div class="panel-heading" style="display: flex; justify-content: space-between; align-items: center;">
						<div>
							<strong>LABORATORY SAFETY AUDIT - Audit Summary</strong>
						</div>
						<div>
							<a class="btn btn-success" target="_blank" data-placement="bottom" data-toggle="tooltip"
								title="Download detailed audit report"
								href="<?php echo base_url($this->uri->segment(1)) . '/overall_lab_safety_audit' ?>">
						...
						<table class="ppeauditallfeedbacks table table-striped table-hover table-bordered" cellspacing="0" width="100%">
							...
							<tbody>
								...
								?>
									...
									<tr class="<?php echo ($sl & 1) ? 'odd gradeX' : 'even gradeC'; ?>" onclick="window.location='<?php echo $lab_safety_audit_feedback . $id; ?>';" style="cursor: pointer;">
										<td><?php echo $sl; ?></td>
										...
										<?php if (allfeedbacks_page('feedback_id') == true) { ?>
											<td>
												<a href="<?php echo  $lab_safety_audit_feedback . $id; ?>">IPDF-<?php echo $id; ?></a>
											</td>
										<?php } ?>

										<td style="overflow: clip;"><?php echo htmlspecialchars($param->staffname); ?></td>
										<td><?php echo htmlspecialchars($param->idno); ?></td>
										<td><?php echo htmlspecialchars($param->department); ?></td>
										<td>
											<a href="<?php echo $lab_safety_audit_feedback . $id; ?>"
...
<script>
	...
	function resposnsechart(callback) {
		...
		var apiUrl = "https://" + domain + "/analytics_audit_quality/resposnsechart_ppe_audit"; // Replace with your API endpoint
...
<script>
	function printChart() {
		...
		<body>
			<h3>LABORATORY SAFETY AUDIT REPORT</h3>
			<p>${reportText}</p>
...
<script>
	function downloadChartImage() {
		...
		img.onload = function() {
			...
			link.download = 'LABORATORY SAFETY AUDIT REPORT.png';
			link.click();
		};
		img.src = chartImage;
	}
