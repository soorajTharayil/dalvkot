...
$int_tickets_count = $this->isr_model->feedback_and_ticket($table_feedback, $table_tickets, $sorttime);
// // tooltips
$int_open_tickets = $this->isr_model->tickets_feeds($table_feedback, $table_tickets, $sorttime, $open);
$int_reopen_tickets = $this->isr_model->tickets_feeds($table_feedback, $table_tickets, $sorttime, $reopen);

$int_closed_tickets = $this->isr_model->tickets_feeds($table_feedback, $table_tickets, $sorttime, $closed);
$int_addressed_tickets = $this->isr_model->tickets_feeds($table_feedback, $table_tickets, $sorttime, $addressed);
...
// // IP ANALYTICS
// $int_nps = $this->isr_model->nps_analytics($table_feedback, $asc, $setup);
// $int_nps_tool = 'Promoters: ' . ($int_nps['promoters_count']) . ', ' . "Detractors: " . ($int_nps['detractors_count']) . ', ' . "Passives: " . ($int_nps['passives_count']);
// $int_psat = $this->isr_model->psat_analytics($table_patients, $table_feedback, $table_tickets, $sorttime);
...
// $int_satisfied_count = $this->isr_model->get_satisfied_count($table_feedback, $table_tickets);
// $int_unsatisfied_count = $this->isr_model->get_unsatisfied_count($table_feedback, $table_tickets);

// // Key Highlights
// $key_highlights = $this->isr_model->key_highlights($table_feedback, $asc, $setup);
...
$ticket_analisys = $this->isr_model->tickets_recived_by_department_interim($type, $table_feedback, $table_tickets);
...
$reject = $this->ticketsesr_model->rejecttickets();
$assign = $this->ticketsesr_model->assignedtickets();

$isr_department['alltickets'] = count($alltickets);
$isr_department['opentickets'] = count($opentickets);
$isr_department['closedticket'] = count($closedtickets);
$isr_department['addressedtickets'] = count($addressed);
$isr_department['rejecttickets'] = count($reject);
$isr_department['assignedtickets'] = count($assign);
...
$int_link_assignedtickets = base_url($this->uri->segment(1) . '/assignedtickets');
// tickets
