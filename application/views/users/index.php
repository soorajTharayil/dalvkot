                            ...
                            <table class="datatable table table-striped table-bordered" cellspacing="0" width="100%">
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th><?php echo lang_loader('global','global_user_name'); ?></th>
                                        <th><?php echo lang_loader('global','global_contact_details'); ?></th>
                                        <th><?php echo lang_loader('global','global_user_role_access'); ?></th>
                                        <th><?php echo lang_loader('global','global_action'); ?></th>
                                    </tr>
                                </thead>
                                <!-- table head end  -->
                                <!-- table body start  -->
                                        ...
                                        <?php foreach ($departments as $department) { ?>
                                            ...
                                            <?php if ($department->user_id >= 2) { ?>
                                                <tr class="<?php echo ($sl & 1) ? "odd gradeX" : "even gradeC" ?>">
                                                    ...
                                                    <td><?php echo $department->email; ?><br><?php echo $department->mobile; ?><br><?php echo $department->alternate_email; ?><br><?php echo $department->alternate_mobile; ?></td>
                                                    <td><b> <?php echo $permission['userrole']; ?></b> <br>
                                                        ...
                                                        if ($department->user_role == 3) {
                                                            ...
                                                            if (ismodule_active('GRIEVANCE') === true && $permission['grievancepermission']) {
                                                                echo 'GRIEVANCE MODULES';
                                                                echo '<br>';
                                                            }
                                                            if ($permission['adf_collect'] || $permission['ip_collect'] || $permission['pcf_collect'] || $permission['op_collect'] || $permission['isr_collect'] || $permission['inc_collect'] || $permission['sg_collect']) {
                                                                echo '<br>';
                                                                //app permission
                                                                echo '<b>Form access given to</b>';
                                                                echo '<br>';

                                                                if (ismodule_active('ADF') === true && $permission['adf_collect']) {
                                                                    echo 'ADMISSION MODULES';
                                                                    echo '<br>';
                                                                }
                                                                if (ismodule_active('IP') === true && $permission['ip_collect']) {
                                                                    echo 'IP MODULES';
                                                                    echo '<br>';
                                                                }
                                                                if (ismodule_active('PCF') === true && $permission['pcf_collect']) {
                                                                    echo 'PC MODULES';
                                                                    echo '<br>';
                                                                }
                                                                if (ismodule_active('OP') === true && $permission['op_collect']) {
                                                                    echo 'OP MODULES';
                                                                    echo '<br>';
                                                                }

                                                                if (ismodule_active('ISR') === true && $permission['isr_collect']) {
                                                                    echo 'ISR MODULES';
                                                                    echo '<br>';
                                                                }
                                                                if (ismodule_active('INCIDENT') === true && $permission['inc_collect']) {
                                                                    echo 'INCIDENT MODULES';
                                                                    echo '<br>';
                                                                }
                                                                if (ismodule_active('GRIEVANCE') === true && $permission['sg_collect']) {
                                                                    echo 'GRIEVANCE MODULES';
                                                                    echo '<br>';
                                                                }
                                                            }
                                                            if ($permission['admin_patient_discharge'] || $permission['admin_allaccess']) {
                                                                echo '<br>';

                                                                echo '<b>Admssion control </b>';
                                                                echo '<br>';
                                                                if ($permission['admin_allaccess']) {
                                                                    echo 'ALL ACCESS';
                                                                    echo '<br>';
                                                                }
                                                                if ($permission['admin_patient_discharge']) {
                                                                    echo 'PATIENT DISCHARGE';
                                                                    echo '<br>';
                                                                }
                                                            }
                                                            if (is_array($permission['floor_ward'])) {
                                                                echo '<br>';
                                                                echo '<b>Floorwise control </b>';
                                                                echo '<br>';
                                                                foreach ($permission['floor_ward'] as $floor) {
                                                            ...
                                                            if (ismodule_active('GRIEVANCE') === true) {
                                                                ...
                                                                foreach ($permission['depgrievance'] as $grievance => $gree) {
                                                                    $this->db->where('dprt_id', $grievance);
                                                                    $query = $this->db->get('department');
                                                                    $result = $query->result();
                                                                    foreach ($result as $row) {
                                                                        // echo strtoupper($row->type);
                                                                        echo strtoupper('grievance');
                                                                        echo ' MODULES';
                                                                        echo ' - ';
                                                                        echo $row->description;
                                                                        echo '<br>';
