						...
						<div class="panel panel-default">
							...
							<div class="panel-body" style="background: #fff;">
								<table class=" table table-striped table-bordered  no-footer dtr-inline ">
									...
									<?php } ?>
									<?php if (($param['consultant_cat']) && $param['consultant_cat'] != '-') { ?>
										<tr>
											<td><b>Speciality</b></td>
											<td><?php echo $param['consultant_cat']; ?></td>
										</tr>
									<?php } ?>
								...
								<div style="padding:10px; background:#fff;">
									<?php foreach ($questioarray as $k => $paramkey) { ?>
										<?php foreach ($paramkey as $key => $r) { ?>
											<?php if ($param[$key] * 1 > 0) { ?>
												...
												<?php
												$negRate = ($param[$key] == 1) ? 'worst' : 'poor';

												if ($param[$key] <= 2) {
													if (array_count_values($param['reason']) >= 1) {
												?>
														<ul class="feedback">
															<li id="feedback">
																<?php if (is_mobile() === true) { ?>
									...
									<ul class="feedback">
										...
										<?php if ($param['overallScore']) { ?>
											<li id="feedback">
												<p id="feedback">
													<span class="fa fa-star <?php if ($param['overallScore'] >= 1) {
																				echo 'checked';
																			} ?>"></span>
										...
										<li id="feedback">
											...
											<p>
												<?php
												$netpromoterscore = $param['recommend1Score'] * 2;
											...
											<?php if ((($param['location'] == 1)) || ($param['specificservice'] == 1) || ($param['referred'] == 1) || ($param['friend'] == 1) || ($param['previous'] == 1) || ($param['docAvailability'] == 1) || ($param['companyRecommend'] == 1) || ($param['otherReason'] == 1)) { ?>
												...
												<div>
													<?php if ($param['location'] == 1) { ?>
														<div><?php echo lang_loader('ip', 'ip_location_proximity'); ?></div>
													<?php } ?>
													<?php if ($param['specificservice'] == 1) { ?>
														<div><?php echo lang_loader('ip', 'ip_specific_service'); ?></div>
													<?php } ?>
													<?php if ($param['referred'] == 1) { ?>
														<div><?php echo lang_loader('ip', 'ip_referred_by_doctors'); ?></div>
													<?php } ?>
													<?php if ($param['friend'] == 1) { ?>
														<div><?php echo lang_loader('ip', 'ip_ff_rocommendation'); ?></div>
													<?php } ?>
													<?php if ($param['previous'] == 1) { ?>
														<div><?php echo lang_loader('ip', 'ip_previous_experience'); ?></div>
													<?php } ?>
													<?php if ($param['docAvailability'] == 1) { ?>
														<div><?php echo lang_loader('ip', 'ip_insurance_facilities'); ?></div>
													<?php } ?>
													<?php if ($param['companyRecommend'] == 1) { ?>
														<div><?php echo lang_loader('ip', 'ip_company_recommendation'); ?></div>
													<?php } ?>
													<?php if ($param['otherReason'] == 1) { ?>
														<div><?php echo lang_loader('ip', 'ip_print_online'); ?></div>
													<?php } ?>
												</div>
											<?php } ?>
											<?php if (!empty($param['staffname'])) { ?>
												<li>
													<?php if (is_mobile() === true) { ?>
