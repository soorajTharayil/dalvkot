            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3><a href="javascript:void()" data-toggle="tooltip" title="<?php echo lang_loader('ip', 'ip_discharge_feedback_tooltip'); ?>">
                            <i class="fa fa-question-circle" aria-hidden="true"></i></a>&nbsp;29. PSQ4d - <?php echo $row->id; ?></h3>
                ...
                <div class="panel-body" style="background: #fff;">
                    ...
                    <?php echo form_open_multipart('quality/edit_feedback_29PSQ3a_byid/' . $this->uri->segment(3), 'class="form-inner"') ?>
                    ...
                    <table class="table table-striped table-bordered no-footer dtr-inline collapsed">
                            ...
                            <td>
                                <div style="display: flex; flex-direction: row; align-items: center;">
                                    <span class="has-float-label">
                                        <input class="form-control" value="<?php echo $param['initial_assessment_hr']; ?>" oninput="restrictToNumerals(event); calculateNearMissesRate();" type="number" id="formula_para1_hr" style="padding-top: 2px; padding-left: 6px; margin-top: 9px; width: 90%;" />
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <br>
                                <button type="button" class="btn btn-primary" onclick="calculateNearMissesRate()">
...
<script>
    ...
    document.querySelector('button[onclick="calculateNearMissesRate()"]').addEventListener('click', calculateNearMissesRate);

    function calculateNearMissesRate() {
        var nearMisses = parseInt(document.getElementById('formula_para1_hr').value);
        var incidentsReported = parseInt(document.getElementById('total_admission').value);

        document.querySelector('input[name="initial_assessment_hr"]').value = nearMisses;
        document.querySelector('input[name="total_admission"]').value = incidentsReported;
        ...
        if (nearMisses > incidentsReported) {
            alert("The no. of near misses reported must be less than the no. of incidents reported.");
            return;
        }

        var calculatedResult;

        // Case 1: Both zero → result = 0
        if (nearMisses === 0 && incidentsReported === 0) {
            calculatedResult = 0;
        }
        // Case 2: Denominator = 0 but numerator > 0 → result = 0 (avoid division by zero)
        else if (incidentsReported === 0) {
            calculatedResult = 0;
        } else {
            // Calculate the percentage of near misses
            calculatedResult = ((nearMisses / incidentsReported) * 100).toFixed(2);
        }
