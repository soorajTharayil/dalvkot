            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3><a href="javascript:void()" data-toggle="tooltip" title="<?php echo lang_loader('ip', 'ip_discharge_feedback_tooltip'); ?>">
                            <i class="fa fa-question-circle" aria-hidden="true"></i></a>&nbsp;9. PSQ3a - <?php echo $row->id; ?></h3>
                ...
                <div class="panel-body" style="background: #fff;">
                    ...
                    <?php echo form_open_multipart('quality/edit_feedback_9PSQ3a_byid/' . $this->uri->segment(3), 'class="form-inner"') ?>
                    ...
                    <table class="table table-striped table-bordered no-footer dtr-inline collapsed">
                            ...
                            <td>
                                <div style="display: flex; flex-direction: row; align-items: center;">
                                    <span class="has-float-label">
                                        <input class="form-control" value="<?php echo $param['initial_assessment_hr']; ?>" name="initial_assessment_hr" oninput="restrictToNumerals(event); calculateTransfusionReactionRate();" type="number" id="formula_para1_hr" style="padding-top: 2px; padding-left: 6px; margin-top: 9px; width: 90%;" />
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <br>
                                <button type="button" class="btn btn-primary" onclick="calculateTransfusionReactionRate()">
                                    <input type="hidden" id="formattedTime" name="formattedTime" value="">


                                    Calculate transfusion reactions
                                </button>
                            </td>
                        </tr>
                        <tr>
...
<script>
    ...
    document.querySelector('button[onclick="calculateTransfusionReactionRate()"]').addEventListener('click', calculateTransfusionReactionRate);

    function calculateTransfusionReactionRate() {
        var transfusionReactions = parseInt(document.getElementById('formula_para1_hr').value);
        var totalUnitsTransfused = parseInt(document.getElementById('total_admission').value);

        document.querySelector('input[name="initial_assessment_hr"]').value = transfusionReactions;
        document.querySelector('input[name="total_admission"]').value = totalUnitsTransfused;
        ...
        if (totalUnitsTransfused === 0) {
            document.getElementById('calculatedResult').value = "0";
            calculationDone = true;
            return;
        }

        // numerator > denominator check unchanged
        if (transfusionReactions > totalUnitsTransfused) {
            alert("Number of transfusion reactions must be less than the number of units transfused.");
            return;
        }

        // Calculate percentage
        var transfusionReactionRate = (transfusionReactions / totalUnitsTransfused) * 100;

        var calculatedResult = transfusionReactionRate % 1 === 0 ?
            transfusionReactionRate.toString() :
            transfusionReactionRate.toFixed(2);
