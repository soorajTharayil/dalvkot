            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3><a href="javascript:void()" data-toggle="tooltip" title="<?php echo lang_loader('ip', 'ip_discharge_feedback_tooltip'); ?>">
                            <i class="fa fa-question-circle" aria-hidden="true"></i></a>&nbsp;15. PSQ3b - <?php echo $row->id; ?></h3>
                ...
                <div class="panel-body" style="background: #fff;">
                    ...
                    <?php echo form_open_multipart('quality/edit_feedback_15PSQ3a_byid/' . $this->uri->segment(3), 'class="form-inner"') ?>
                    ...
                    <table class="table table-striped table-bordered no-footer dtr-inline collapsed">
                            ...
                            <td>
                                <div style="display: flex; flex-direction: row; align-items: center;">
                                    <span class="has-float-label">
                                        <input class="form-control" value="<?php echo $param['initial_assessment_hr']; ?>" name="initial_assessment_hr" oninput="restrictToNumerals(event); calculateCLABSIRate();" type="number" id="formula_para1_hr" style="padding-top: 2px; padding-left: 6px; margin-top: 9px; width: 90%;" />
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <br>
                                <button type="button" class="btn btn-primary" onclick="calculateCLABSIRate()">
                                    <input type="hidden" id="formattedTime" name="formattedTime" value="">


                                    Calculate blood stream infection
                                </button>
                            </td>
                        </tr>
                        <tr>
...
<script>
    ...
    document.querySelector('button[onclick="calculateCLABSIRate()"]').addEventListener('click', calculateCLABSIRate);

    function calculateCLABSIRate() {
        var centralLineInfections = parseInt(document.getElementById('formula_para1_hr').value);
        var centralLineDays = parseInt(document.getElementById('total_admission').value);

        document.querySelector('input[name="initial_assessment_hr"]').value = centralLineInfections;
        document.querySelector('input[name="total_admission"]').value = centralLineDays;

        // Validate numerator (allow 0)
        if (isNaN(centralLineInfections) || centralLineInfections < 0) {
            alert("Enter the number of central line-associated bloodstream infections.");
            return;
        }
        ...
        // numerator > denominator only when denominator > 0
        if (centralLineDays > 0 && centralLineInfections > centralLineDays) {
            alert("Number of central line-associated bloodstream infections must be less than the number of central line days.");
            return;
        }
        ...
        if (centralLineDays === 0) {
            clabsiRate = 0;
        } else {
            clabsiRate = (centralLineInfections / centralLineDays) * 1000;
        }

        var calculatedResult;
        if (clabsiRate % 1 === 0) {
            calculatedResult = clabsiRate.toString();
        } else {
            calculatedResult = clabsiRate.toFixed(2);
        }
