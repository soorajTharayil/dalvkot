            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3><a href="javascript:void()" data-toggle="tooltip" title="<?php echo lang_loader('ip', 'ip_discharge_feedback_tooltip'); ?>">
                            <i class="fa fa-question-circle" aria-hidden="true"></i></a>&nbsp;18. PSQ3b - <?php echo $row->id; ?></h3>
                ...
                <div class="panel-body" style="background: #fff;">
                    ...
                    <?php echo form_open_multipart('quality/edit_feedback_18PSQ3a_byid/' . $this->uri->segment(3), 'class="form-inner"') ?>
                    ...
                    <table class="table table-striped table-bordered no-footer dtr-inline collapsed">
                            ...
                            <td>
                                <div style="display: flex; flex-direction: row; align-items: center;">
                                    <span class="has-float-label">
                                        <input class="form-control" value="<?php echo $param['initial_assessment_hr']; ?>" name="initial_assessment_hr" oninput="restrictToNumerals(event); calculateProphylacticAntibioticsRate();" type="number" id="formula_para1_hr" style="padding-top: 2px; padding-left: 6px; margin-top: 9px; width: 90%;" />
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <br>
                                <button type="button" class="btn btn-primary" onclick="calculateProphylacticAntibioticsRate()">
                                    <input type="hidden" id="formattedTime" name="formattedTime" value="">


                                    Calculate prophylactic antibiotics
                                </button>
                            </td>
                        </tr>
                        <tr>
...
<script>
    ...
    document.querySelector('button[onclick="calculateProphylacticAntibioticsRate()"]').addEventListener('click', calculateProphylacticAntibioticsRate);

    function calculateProphylacticAntibioticsRate() {
        var patientsWithProphylacticAntibiotics = parseInt(document.getElementById('formula_para1_hr').value);
        var patientsUnderwentSurgeries = parseInt(document.getElementById('total_admission').value);

        document.querySelector('input[name="initial_assessment_hr"]').value = patientsWithProphylacticAntibiotics;
        document.querySelector('input[name="total_admission"]').value = patientsUnderwentSurgeries;

        // Validate numerator (allow 0)
        if (isNaN(patientsWithProphylacticAntibiotics) || patientsWithProphylacticAntibiotics < 0) {
            alert("Enter the no. of patients who received appropriate prophylactic antibiotics.");
            return;
        }

        // Validate denominator (allow 0)
        if (isNaN(patientsUnderwentSurgeries) || patientsUnderwentSurgeries < 0) {
            alert("Enter the no. of patients who underwent surgeries in the OT.");
            return;
        }

        // Prevent numerator > denominator only when denominator > 0
        if (patientsUnderwentSurgeries > 0 && patientsWithProphylacticAntibiotics > patientsUnderwentSurgeries) {
            alert("Number of patients who received prophylactic antibiotics must be less than number of patients who underwent surgeries.");
            return;
        }

        var calculatedResult;

        // If denominator is zero → result = 0
        if (patientsUnderwentSurgeries === 0) {
            calculatedResult = 0;
        } else {
            calculatedResult = ((patientsWithProphylacticAntibiotics / patientsUnderwentSurgeries) * 100).toFixed(2);
        }
