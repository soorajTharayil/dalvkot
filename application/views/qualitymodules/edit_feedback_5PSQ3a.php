            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3><a href="javascript:void()" data-toggle="tooltip" title="<?php echo lang_loader('ip', 'ip_discharge_feedback_tooltip'); ?>">
                            <i class="fa fa-question-circle" aria-hidden="true"></i></a>&nbsp;5. PSQ3a - <?php echo $row->id; ?></h3>
                ...
                <div class="panel-body" style="background: #fff;">
                    ...
                    <?php echo form_open_multipart('quality/edit_feedback_5PSQ3a_byid/' . $this->uri->segment(3), 'class="form-inner"') ?>
                    ...
                    <table class="table table-striped table-bordered no-footer dtr-inline collapsed">
                        <tr>
                            <td><b>Number of medication charts with error prone abbreviation (s)</b></td>
                            <td>
                                <div style="display: flex; flex-direction: row; align-items: center;">
                                    <span class="has-float-label">
                                        <input class="form-control" value="<?php echo $param['initial_assessment_hr']; ?>" name="initial_assessment_hr" oninput="restrictToNumerals(event); calculateAbbreviationErrorRate();" type="number" id="formula_para1_hr" style="padding-top: 2px; padding-left: 6px; margin-top: 9px; width: 90%;" />
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <br>
                                <button type="button" class="btn btn-primary" onclick="calculateAbbreviationErrorRate()">
                                    <input type="hidden" id="formattedTime" name="formattedTime" value="">


                                    Calculate error-prone abbreviation
                                </button>
                            </td>
                        </tr>
                        <tr>
...
<script>
    ...
    document.querySelector('button[onclick="calculateAbbreviationErrorRate()"]').addEventListener('click', calculateAbbreviationErrorRate);


    function calculateAbbreviationErrorRate() {
        var chartsWithErrors = parseInt(document.getElementById('formula_para1_hr').value);
        var chartsReviewed = parseInt(document.getElementById('total_admission').value);

        document.querySelector('input[name="initial_assessment_hr"]').value = chartsWithErrors;
        document.querySelector('input[name="total_admission"]').value = chartsReviewed;

        // Allow numerator = 0
        if (isNaN(chartsWithErrors) || chartsWithErrors < 0) {
            alert("Enter the number of medication charts with error-prone abbreviations.");
            return;
        }
        ...
        if (chartsWithErrors > chartsReviewed) {
            alert("Number of medication charts with error-prone abbreviations must be less than the number of medication charts reviewed.");
            return;
        }
        ...
        var calculatedResult = abbreviationErrorRate % 1 === 0 ?
            abbreviationErrorRate.toString() :
            abbreviationErrorRate.toFixed(2);
