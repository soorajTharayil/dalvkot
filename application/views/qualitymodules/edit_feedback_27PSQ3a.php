            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3><a href="javascript:void()" data-toggle="tooltip" title="<?php echo lang_loader('ip', 'ip_discharge_feedback_tooltip'); ?>">
                            <i class="fa fa-question-circle" aria-hidden="true"></i></a>&nbsp;27. PSQ4d - <?php echo $row->id; ?></h3>
                ...
                <div class="panel-body" style="background: #fff;">
                    ...
                    <?php echo form_open_multipart('quality/edit_feedback_27PSQ3a_byid/' . $this->uri->segment(3), 'class="form-inner"') ?>
                    ...
                    <table class="table table-striped table-bordered no-footer dtr-inline collapsed">
                            ...
                            <td>
                                <div style="display: flex; flex-direction: row; align-items: center;">
                                    <span class="has-float-label">
                                        <input class="form-control" value="<?php echo $param['initial_assessment_hr']; ?>" oninput="restrictToNumerals(event); calculateMockDrillVariationsRate();" type="number" id="formula_para1_hr" style="padding-top: 2px; padding-left: 6px; margin-top: 9px; width: 90%;" />
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <br>
                                <button type="button" class="btn btn-primary" onclick="calculateMockDrillVariationsRate()">
                                    <input type="hidden" id="formattedTime" name="formattedTime" value="">


                                    Calculate mock variations
                                </button>
                            </td>
                        </tr>
                        <tr>
...
<script>
    ...
    document.querySelector('button[onclick="calculateMockDrillVariationsRate()"]').addEventListener('click', calculateMockDrillVariationsRate);

    function calculateMockDrillVariationsRate() {
        var variationsObserved = parseInt(document.getElementById('formula_para1_hr').value);
        var opportunitiesOfVariations = parseInt(document.getElementById('total_admission').value);

        document.querySelector('input[name="initial_assessment_hr"]').value = variationsObserved;
        document.querySelector('input[name="total_admission"]').value = opportunitiesOfVariations;
        ...
        if (variationsObserved > opportunitiesOfVariations) {
            alert("The total no. of variations observed must be less than the total no. of opportunities of variations.");
            return;
        }

        var calculatedResult;

        // ✅ Zero-safe logic (added, nothing else changed)
        if (opportunitiesOfVariations === 0) {
            calculatedResult = 0; // Prevent Infinity
        } else {
            calculatedResult = ((variationsObserved / opportunitiesOfVariations) * 100).toFixed(2);
        }
