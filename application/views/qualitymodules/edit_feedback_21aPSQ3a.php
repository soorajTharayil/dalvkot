            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3><a href="javascript:void()" data-toggle="tooltip" title="<?php echo lang_loader('ip', 'ip_discharge_feedback_tooltip'); ?>">
                            <i class="fa fa-question-circle" aria-hidden="true"></i></a>&nbsp;21a. PSQ3c - <?php echo $row->id; ?></h3>
                ...
                <div class="panel-body" style="background: #fff;">
                    ...
                    <?php echo form_open_multipart('quality/edit_feedback_21aPSQ3a_byid/' . $this->uri->segment(3), 'class="form-inner"') ?>
...
<script>
    ...
    function calculateNursePatientRatio() {
        ...
        // --- ZERO HANDLING ADDED ---
        // Case 1: both zero → result is 0:0
        if ((nursingStaff === 0 || isNaN(nursingStaff)) && (occupiedBeds === 0 || isNaN(occupiedBeds))) {
            document.getElementById('calculatedResult').value = "0:0";
            calculationDone = true;
            return;
        }

        // Case 2: numerator zero only → 1:∞
        if (nursingStaff === 0 || isNaN(nursingStaff)) {
            document.getElementById('calculatedResult').value = "1:∞";
            calculationDone = true;
            return;
        }

        // Case 3: denominator zero only → ∞:1
        if (occupiedBeds === 0 || isNaN(occupiedBeds)) {
            document.getElementById('calculatedResult').value = "∞:1";
            calculationDone = true;
            return;
        }
