            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3><a href="javascript:void()" data-toggle="tooltip" title="<?php echo lang_loader('ip', 'ip_discharge_feedback_tooltip'); ?>">
                            <i class="fa fa-question-circle" aria-hidden="true"></i></a>&nbsp;17. PSQ3b - <?php echo $row->id; ?></h3>
                ...
                <div class="panel-body" style="background: #fff;">
                    ...
                    <?php echo form_open_multipart('quality/edit_feedback_17PSQ3a_byid/' . $this->uri->segment(3), 'class="form-inner"') ?>
                    ...
                    <table class="table table-striped table-bordered no-footer dtr-inline collapsed">
                            ...
                            <td>
                                <div style="display: flex; flex-direction: row; align-items: center;">
                                    <span class="has-float-label">
                                        <input class="form-control" value="<?php echo $param['initial_assessment_hr']; ?>" name="initial_assessment_hr" oninput="restrictToNumerals(event); calculateHandHygieneComplianceRate();" type="number" id="formula_para1_hr" style="padding-top: 2px; padding-left: 6px; margin-top: 9px; width: 90%;" />
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <br>
                                <button type="button" class="btn btn-primary" onclick="calculateHandHygieneComplianceRate()">
                                    <input type="hidden" id="formattedTime" name="formattedTime" value="">


                                    Calculate hygiene compliance rate
                                </button>
                            </td>
                        </tr>
                        <tr>
...
<script>
    ...
    document.querySelector('button[onclick="calculateHandHygieneComplianceRate()"]').addEventListener('click', calculateHandHygieneComplianceRate);

    function calculateHandHygieneComplianceRate() {
        var handHygieneActions = parseInt(document.getElementById('formula_para1_hr').value);
        var handHygieneOpportunities = parseInt(document.getElementById('total_admission').value);

        document.querySelector('input[name="initial_assessment_hr"]').value = handHygieneActions;
        document.querySelector('input[name="total_admission"]').value = handHygieneOpportunities;
        ...
        // Validate denominator (allow 0 now)
        if (isNaN(handHygieneOpportunities) || handHygieneOpportunities < 0) {
            alert("Enter the total number of hand hygiene opportunities.");
            return;
        }

        // Prevent numerator > denominator only if denominator > 0
        if (handHygieneOpportunities > 0 && handHygieneActions > handHygieneOpportunities) {
            alert("Number of hand hygiene actions must be less than the number of hand hygiene opportunities.");
            return;
        }
        ...
        if (handHygieneOpportunities === 0) {
            calculatedResult = 0;
        } else {
            calculatedResult = ((handHygieneActions / handHygieneOpportunities) * 100).toFixed(2);
        }
