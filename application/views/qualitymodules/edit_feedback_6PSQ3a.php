            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3><a href="javascript:void()" data-toggle="tooltip" title="<?php echo lang_loader('ip', 'ip_discharge_feedback_tooltip'); ?>">
                            <i class="fa fa-question-circle" aria-hidden="true"></i></a>&nbsp;6. PSQ3a - <?php echo $row->id; ?></h3>
                ...
                <div class="panel-body" style="background: #fff;">
                    ...
                    <?php echo form_open_multipart('quality/edit_feedback_6PSQ3a_byid/' . $this->uri->segment(3), 'class="form-inner"') ?>
                            ...
                            <td>
                                <div style="display: flex; flex-direction: row; align-items: center;">
                                    <span class="has-float-label">
                                        <input class="form-control" value="<?php echo $param['initial_assessment_hr']; ?>" name="initial_assessment_hr" oninput="restrictToNumerals(event); calculateAdverseDrugReactionRate();" type="number" id="formula_para1_hr" style="padding-top: 2px; padding-left: 6px; margin-top: 9px; width: 90%;" />
...
<script>
    ...
    document.querySelector('button[onclick="calculateAdverseDrugReactionRate()"]').addEventListener('click', calculateAdverseDrugReactionRate);


    function calculateAdverseDrugReactionRate() {
        var patientsWithReactions = parseInt(document.getElementById('formula_para1_hr').value);
        var totalInPatients = parseInt(document.getElementById('total_admission').value);

        document.querySelector('input[name="initial_assessment_hr"]').value = patientsWithReactions;
        document.querySelector('input[name="total_admission"]').value = totalInPatients;

        // Allow numerator = 0 but not negative
        if (isNaN(patientsWithReactions) || patientsWithReactions < 0) {
            alert("Enter the number of patients developing adverse drug reactions.");
            return;
        }
        ...
        if (patientsWithReactions > totalInPatients) {
            alert("Number of patients developing adverse drug reactions must be less than the number of in-patients.");
            return;
        }

        // Calculate the percentage
        var adverseDrugReactionRate = (patientsWithReactions / totalInPatients) * 100;

        var calculatedResult =
            adverseDrugReactionRate % 1 === 0 ?
            adverseDrugReactionRate.toString() :
            adverseDrugReactionRate.toFixed(2);
