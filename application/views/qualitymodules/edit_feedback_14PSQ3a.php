            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3><a href="javascript:void()" data-toggle="tooltip" title="<?php echo lang_loader('ip', 'ip_discharge_feedback_tooltip'); ?>">
                            <i class="fa fa-question-circle" aria-hidden="true"></i></a>&nbsp;14. PSQ3b - <?php echo $row->id; ?></h3>
                ...
                <div class="panel-body" style="background: #fff;">
                    ...
                    <?php echo form_open_multipart('quality/edit_feedback_14PSQ3a_byid/' . $this->uri->segment(3), 'class="form-inner"') ?>
                    ...
                    <table class="table table-striped table-bordered no-footer dtr-inline collapsed">
                            ...
                            <td>
                                <div style="display: flex; flex-direction: row; align-items: center;">
                                    <span class="has-float-label">
                                        <input class="form-control" value="<?php echo $param['initial_assessment_hr']; ?>" name="initial_assessment_hr" oninput="restrictToNumerals(event); calculateVAPRate();" type="number" id="formula_para1_hr" style="padding-top: 2px; padding-left: 6px; margin-top: 9px; width: 90%;" />
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <br>
                                <button type="button" class="btn btn-primary" onclick="calculateVAPRate()">
                                    <input type="hidden" id="formattedTime" name="formattedTime" value="">


                                    Calculate pneumoniarate rate
                                </button>
                            </td>
                        </tr>
                        <tr>
...
<script>
    ...
    document.querySelector('button[onclick="calculateVAPRate()"]').addEventListener('click', calculateVAPRate);

    function calculateVAPRate() {
        var ventilatorAssociatedPneumonia = parseInt(document.getElementById('formula_para1_hr').value);
        var ventilatorDays = parseInt(document.getElementById('total_admission').value);

        document.querySelector('input[name="initial_assessment_hr"]').value = ventilatorAssociatedPneumonia;
        document.querySelector('input[name="total_admission"]').value = ventilatorDays;
        ...
        if (ventilatorDays > 0 && ventilatorAssociatedPneumonia > ventilatorDays) {
            alert("Number of ventilator associated pneumonia must be less than the number of ventilator days.");
            return;
        }

        var calculatedResult;

        // If denominator is zero → result = 0 (safe)
        if (ventilatorDays === 0) {
            calculatedResult = 0;
        } else {
            calculatedResult = Math.round((ventilatorAssociatedPneumonia / ventilatorDays) * 1000);
        }
