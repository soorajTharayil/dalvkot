...
<script>
    ...
    function calculateAdherenceRate() {
        ...
        var adheringStaffInput = document.getElementById('initial_assessment_hr').value;
        var auditedStaffInput = document.getElementById('total_admission').value;

        // Allow ZERO but not EMPTY
        if (adheringStaffInput === "") {
            alert("Enter the number of staff adhering to safety precautions.");
            return;
        }

        if (auditedStaffInput === "") {
            alert("Enter the number of staff audited.");
            return;
        }

        var adheringStaff = parseInt(adheringStaffInput) || 0;
        var auditedStaff = parseInt(auditedStaffInput) || 0;
        ...
        // If denominator is zero → result must be 0%
        if (auditedStaff === 0) {
        ...
        // If numerator > denominator (valid only when denominator >0)
        if (auditedStaff !== 0 && adheringStaff > auditedStaff) {
