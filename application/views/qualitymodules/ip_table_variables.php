...
/* quality_model.php FOR GLOBAL UPDATES */
...
$ip_tickets_count = $this->quality_model->feedback_and_ticket($table_feedback, $table_tickets, $sorttime);
// tooltips
$ip_open_tickets = $this->quality_model->tickets_feeds($table_feedback, $table_tickets, $sorttime, $open);
$ip_reopen_tickets = $this->quality_model->tickets_feeds($table_feedback, $table_tickets, $sorttime, $reopen);

$ip_closed_tickets = $this->quality_model->tickets_feeds($table_feedback, $table_tickets, $sorttime, $closed);
$ip_addressed_tickets = $this->quality_model->tickets_feeds($table_feedback, $table_tickets, $sorttime, $addressed);
...
// $ip_department = $this->departmenthead_model->departmenthead_values($table_patients, $table_feedback, $table_tickets, $sorttime, $department, $setup, $type);
$ticket_analisys = $this->quality_model->tickets_recived_by_department($type, $table_feedback, $table_tickets);
...
$close_rate = $this->quality_model->ticket_rate($table_tickets, $status, $table_feedback, $table_ticket_action);
$ticket_close_rate = secondsToTime($close_rate);



// $close_rate = $this->quality_model->ticket_rate($table_tickets, $closed, $table_feedback, $table_ticket_action);


$ip_satisfied_count = $this->quality_model->get_satisfied_count($table_feedback, $table_tickets);
$ip_unsatisfied_count = $this->quality_model->get_unsatisfied_count($table_feedback, $table_tickets);

// Key Highlights
$key_highlights = $this->quality_model->key_highlights($table_patients, $table_feedback, $sorttime, $setup);


$selectionarray = $this->quality_model->reason_to_choose_hospital($table_feedback, $sorttime);
// $close_rate = $this->quality_model->ticket_rate($table_tickets, $table_ticket_action, $closed);
// $close_time = $this->quality_model->convertSecondsToTime($close_rate);
