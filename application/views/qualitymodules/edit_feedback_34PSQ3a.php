        ...
        <div class="col-lg-12">
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body" style="background: #fff;">
                    ...
                    <?php echo form_open_multipart('quality/edit_feedback_34PSQ3a_byid/' . $this->uri->segment(3), 'class="form-inner"') ?>
...
<script>
    ...
    function calculateAdverseDrugReactionRate() {
        var prescriptionsInCapitals = parseInt(document.getElementById('formula_para1_hr').value);
        var totalPrescriptions = parseInt(document.getElementById('total_admission').value);
        ...
        if (isNaN(prescriptionsInCapitals) || prescriptionsInCapitals < 0) {
            alert("Enter no. of AMI patients during primary angioplasty for whom door to balloon time of 90 minutes is achieved");
            return;
        }

        // Allow zero (changed only here)
        if (isNaN(totalPrescriptions) || totalPrescriptions < 0) {
            alert("Enter total no. of AMI patients undergoing primary angioplasty");
            return;
        }

        if (prescriptionsInCapitals > totalPrescriptions) {
            alert("Enter no. of AMI patients with door-to-balloon time within 90 min must be less than total AMI patients undergoing primary angioplasty");
            return;
        }
        ...
        if (prescriptionsInCapitals === 0 && totalPrescriptions === 0) {
            calculatedResult = 0;
        }
        // If denominator is zero → return 0 (avoid divide-by-zero)
        else if (totalPrescriptions === 0) {
            calculatedResult = 0;
        } else {
            // Normal calculation
            var complianceRate = (prescriptionsInCapitals / totalPrescriptions) * 100;
            calculatedResult = complianceRate.toFixed(2);
        }
