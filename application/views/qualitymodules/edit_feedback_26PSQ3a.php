            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3><a href="javascript:void()" data-toggle="tooltip" title="<?php echo lang_loader('ip', 'ip_discharge_feedback_tooltip'); ?>">
                            <i class="fa fa-question-circle" aria-hidden="true"></i></a>&nbsp;26. PSQ4c - <?php echo $row->id; ?></h3>
                ...
                <div class="panel-body" style="background: #fff;">
                    ...
                    <?php echo form_open_multipart('quality/edit_feedback_26PSQ3a_byid/' . $this->uri->segment(3), 'class="form-inner"') ?>
                    ...
                    <table class="table table-striped table-bordered no-footer dtr-inline collapsed">
                            ...
                            <td>
                                <div style="display: flex; flex-direction: row; align-items: center;">
                                    <span class="has-float-label">
                                        <input class="form-control" value="<?php echo $param['initial_assessment_hr']; ?>" oninput="restrictToNumerals(event); calculateStockOutRate();" type="number" id="formula_para1_hr" style="padding-top: 2px; padding-left: 6px; margin-top: 9px; width: 90%;" />
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <br>
                                <button type="button" class="btn btn-primary" onclick="calculateStockOutRate()">
...
<script>
    ...
    document.querySelector('button[onclick="calculateStockOutRate()"]').addEventListener('click', calculateStockOutRate);

    function calculateStockOutRate() {
        var stockOuts = parseInt(document.getElementById('formula_para1_hr').value);
        var emergencyDrugsListed = parseInt(document.getElementById('total_admission').value);

        document.querySelector('input[name="initial_assessment_hr"]').value = stockOuts;
        document.querySelector('input[name="total_admission"]').value = emergencyDrugsListed;
        ...
        if (isNaN(emergencyDrugsListed) || emergencyDrugsListed < 0) {
            alert("Enter the number of drugs listed as emergency drugs in hospital formulary.");
            return;
        }

        if (stockOuts > emergencyDrugsListed) {
            alert("The no. of stock outs of emergency drugs must be less than the no. of drugs listed as emergency drugs.");
            return;
        }

        var calculatedResult;

        // ✅ Zero-safe logic added (no other logic touched)
        if (emergencyDrugsListed === 0) {
            calculatedResult = 0; // Prevent Infinity or NaN
        } else {
            calculatedResult = ((stockOuts / emergencyDrugsListed) * 100).toFixed(2);
        }
