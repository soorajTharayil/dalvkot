            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3><a href="javascript:void()" data-toggle="tooltip" title="<?php echo lang_loader('ip', 'ip_discharge_feedback_tooltip'); ?>">
                            <i class="fa fa-question-circle" aria-hidden="true"></i></a>&nbsp;8. PSQ3a - <?php echo $row->id; ?></h3>
                ...
                <div class="panel-body" style="background: #fff;">
                    ...
                    <?php echo form_open_multipart('quality/edit_feedback_8PSQ3a_byid/' . $this->uri->segment(3), 'class="form-inner"') ?>
                    ...
                    <table class="table table-striped table-bordered no-footer dtr-inline collapsed">
                            ...
                            <td>
                                <div style="display: flex; flex-direction: row; align-items: center;">
                                    <span class="has-float-label">
                                        <input class="form-control" value="<?php echo $param['initial_assessment_hr']; ?>" name="initial_assessment_hr" oninput="restrictToNumerals(event); calculateAdherenceToProcedureRate();" type="number" id="formula_para1_hr" style="padding-top: 2px; padding-left: 6px; margin-top: 9px; width: 90%;" />
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <br>
                                <button type="button" class="btn btn-primary" onclick="calculateAdherenceToProcedureRate()">
                                    <input type="hidden" id="formattedTime" name="formattedTime" value="">


                                    Calculate surgical adherence
                                </button>
                            </td>
                        </tr>
                        <tr>
                            <td><b>Percentage of surgeries where the organisation's procedure to prevent adverse events have been adhered to</b></td>
                            <td>
...
<script>
    ...
    document.querySelector('button[onclick="calculateAdherenceToProcedureRate()"]').addEventListener('click', calculateAdherenceToProcedureRate);

    function calculateAdherenceToProcedureRate() {
        var surgeriesWithProcedureFollowed = parseInt(document.getElementById('formula_para1_hr').value);
        var totalSurgeriesPerformed = parseInt(document.getElementById('total_admission').value);

        document.querySelector('input[name="initial_assessment_hr"]').value = surgeriesWithProcedureFollowed;
        document.querySelector('input[name="total_admission"]').value = totalSurgeriesPerformed;

        // Allow numerator = 0 but not negative
        if (isNaN(surgeriesWithProcedureFollowed) || surgeriesWithProcedureFollowed < 0) {
            alert("Enter the number of surgeries where the procedure was followed.");
            return;
        }

        // Allow denominator = 0 but not negative
        if (isNaN(totalSurgeriesPerformed) || totalSurgeriesPerformed < 0) {
            alert("Enter the total number of surgeries performed.");
            return;
        }

        // If denominator = 0 → result = 0
        if (totalSurgeriesPerformed === 0) {
            document.getElementById('calculatedResult').value = "0";
        ...
        if (surgeriesWithProcedureFollowed > totalSurgeriesPerformed) {
            alert("Number of surgeries where the procedure was followed must be less than the number of surgeries performed.");
            return;
        }

        // Calculate the percentage of surgeries where the procedure was followed
        var adherenceToProcedureRate = (surgeriesWithProcedureFollowed / totalSurgeriesPerformed) * 100;

        var calculatedResult =
            adherenceToProcedureRate % 1 === 0 ?
            adherenceToProcedureRate.toString() :
            adherenceToProcedureRate.toFixed(2);
