            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3><a href="javascript:void()" data-toggle="tooltip" title="<?php echo lang_loader('ip', 'ip_discharge_feedback_tooltip'); ?>">
                            <i class="fa fa-question-circle" aria-hidden="true"></i></a>&nbsp;28. PSQ4d - <?php echo $row->id; ?></h3>
                ...
                <div class="panel-body" style="background: #fff;">
                    ...
                    <?php echo form_open_multipart('quality/edit_feedback_28PSQ3a_byid/' . $this->uri->segment(3), 'class="form-inner"') ?>
                    ...
                    <table class="table table-striped table-bordered no-footer dtr-inline collapsed">
                            ...
                            <td>
                                <div style="display: flex; flex-direction: row; align-items: center;">
                                    <span class="has-float-label">
                                        <input class="form-control" value="<?php echo $param['initial_assessment_hr']; ?>" oninput="restrictToNumerals(event); calculatePatientFallRate();" type="number" id="formula_para1_hr" style="padding-top: 2px; padding-left: 6px; margin-top: 9px; width: 90%;" />
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <br>
                                <button type="button" class="btn btn-primary" onclick="calculatePatientFallRate()">
                                    <input type="hidden" id="formattedTime" name="formattedTime" value="">


                                    Calculate patient fall rate
                                </button>
                            </td>
                        </tr>
                        <tr>
...
<script>
    ...
    document.querySelector('button[onclick="calculatePatientFallRate()"]').addEventListener('click', calculatePatientFallRate);

    function calculatePatientFallRate() {
        var patientFalls = parseInt(document.getElementById('formula_para1_hr').value);
        var patientDays = parseInt(document.getElementById('total_admission').value);

        document.querySelector('input[name="initial_assessment_hr"]').value = patientFalls;
        document.querySelector('input[name="total_admission"]').value = patientDays;
        ...
        if (patientFalls > patientDays) {
            alert("The number of patient falls must be less than the total number of patient days.");
            return;
        }
        ...
        // NEW: Allow numerator > 0 and denominator = 0 (avoid division by zero)
        else if (patientDays === 0) {
            calculatedResult = 0; // Or any value you want instead of Infinity
        } else {
            // Calculate the patient fall rate per 1000 patient days
            calculatedResult = Math.round((patientFalls / patientDays) * 1000);
        }
