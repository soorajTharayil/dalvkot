            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3><a href="javascript:void()" data-toggle="tooltip" title="<?php echo lang_loader('ip', 'ip_discharge_feedback_tooltip'); ?>">
                            <i class="fa fa-question-circle" aria-hidden="true"></i></a>&nbsp;13. PSQ3b - <?php echo $row->id; ?></h3>
                ...
                <div class="panel-body" style="background: #fff;">
                    ...
                    <?php echo form_open_multipart('quality/edit_feedback_13PSQ3a_byid/' . $this->uri->segment(3), 'class="form-inner"') ?>
                    ...
                    <table class="table table-striped table-bordered no-footer dtr-inline collapsed">
                            ...
                            <td>
                                <div style="display: flex; flex-direction: row; align-items: center;">
                                    <span class="has-float-label">
                                        <input class="form-control" value="<?php echo $param['initial_assessment_hr']; ?>" name="initial_assessment_hr" oninput="restrictToNumerals(event); calculateCAUTIRate();" type="number" id="formula_para1_hr" style="padding-top: 2px; padding-left: 6px; margin-top: 9px; width: 90%;" />
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <br>
                                <button type="button" class="btn btn-primary" onclick="calculateCAUTIRate()">
                                    <input type="hidden" id="formattedTime" name="formattedTime" value="">


                                    Calculate urinary tract infection
                                </button>
                            </td>
                        </tr>
                        <tr>
...
<script>
    ...
    document.querySelector('button[onclick="calculateCAUTIRate()"]').addEventListener('click', calculateCAUTIRate);

    function calculateCAUTIRate() {
        var urinaryCatheterUTIs = parseInt(document.getElementById('formula_para1_hr').value);
        var urinaryCatheterDays = parseInt(document.getElementById('total_admission').value);

        document.querySelector('input[name="initial_assessment_hr"]').value = urinaryCatheterUTIs;
        document.querySelector('input[name="total_admission"]').value = urinaryCatheterDays;
        ...
        // Validate denominator (allow zero now)
        if (isNaN(urinaryCatheterDays) || urinaryCatheterDays < 0) {
            alert("Enter the number of urinary catheter days in that month.");
            return;
        }

        // Check numerator > denominator (only if denominator > 0)
        if (urinaryCatheterDays > 0 && urinaryCatheterUTIs > urinaryCatheterDays) {
        ...
        if (urinaryCatheterDays === 0) {
            calculatedResult = 0;
        } else {
            calculatedResult = Math.round((urinaryCatheterUTIs / urinaryCatheterDays) * 1000);
        }
