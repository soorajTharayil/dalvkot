<div class="content">
	<div class="row">
		...
		$this->db->select('tickets_doctor.*,bf_feedback_doctors.dataset as dataset');
		$this->db->from('tickets_doctor');
		$this->db->join('bf_feedback_doctors', 'bf_feedback_doctors.id = tickets_doctor.feedbackid', 'inner');
		$this->db->join('bf_opatients', 'bf_opatients.id = bf_feedback_doctors.pid', 'inner');
		$this->db->join('department', 'department.dprt_id = tickets_doctor.departmentid', 'inner');
		$this->db->where('bf_feedback_doctors.datet <=', $fdate);
		$this->db->where('bf_feedback_doctors.datet >=', $tdate);
		...
		$this->db->where_in('department.slug', $this->session->userdata['question_array'][$type]);
		$this->db->where('setup_doctor.parent', 0);
		$this->db->where('department.type', 'doctor');
		$query = $this->db->get();
							...
							<a href="<?php echo base_url($this->uri->segment(1)); ?>/track/<?php echo $ticketdata->id ?>">
								<div class="inbox-item">
									<p class="inbox-item-author">
										<?php echo 'DFT-' . $ticketdata->id ?>
										<span style="float: right;font-size:10px;"><?php
																					echo date('g:i A, d-m-y', strtotime($ticketdata->created_on)); ?></span>
									</p>

									<p class="inbox-item-text">
										<i class="fa fa-user-plus"></i> <?php echo $ticketdata->feed->name; ?> (<span style="color:#62c52d;"><?php echo $ticketdata->feed->ot_num; ?></span>), from
										<?php
