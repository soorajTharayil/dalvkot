<div class="content">
	...
	if ($this->input->post('id') || $this->input->get('id')) {
		...
		$this->db->where('id', $id);
		$query = $this->db->get('bf_feedback_doctors');
					...
					<div class="col-lg-12">
						<div class="panel panel-default">
							<div class="panel-heading">
								<h3><a href="javascript:void()" data-toggle="tooltip" title="OT DOCTOR FEEDBACKS- <Feedback ID>"> <i class="fa fa-question-circle" aria-hidden="true"></i></a> ODF <?php echo $result->id; ?> </h3>
							</div>
							<div class="panel-body" style="background: #fff;">
								...
								<table class=" table table-striped table-bordered  no-footer dtr-inline ">
									...
									<tr>
										<td><b>Asisstant Surgeon</b></td>
										<td><?php echo $param['asistant_surgeon']; ?></td>
									</tr>
									<tr>
										<td><b>Scrub Nurse</b></td>
										<td><?php echo $param['scrub_nurse']; ?></td>
									</tr>
									<tr>
										<td><b>Surgery Name</b></td>
										<td><?php echo $param['surgery_name']; ?></td>
									</tr>
									...
									<?php } ?>
									<tr>
										<td><b>OT Room No</b></td>
										<td><?php echo $param['ot_num']; ?></td>
									</tr>
									...
									<?php } ?>
									<tr>
										<td><b>Surgery Start Date & Time</b></td>
										<td>
											<?php if (!empty($param['surgeryStartDate'])) { ?>
												<?php echo date('g:i A', strtotime($param['surgeryStartDate'])); ?>
												<?php echo date('d-m-y', strtotime($param['surgeryStartDate'])); ?>
											<?php } ?>
										</td>
									</tr>

									<tr>
										<td><b>Surgery End Date & Time</b></td>
										<td>
											<?php if (!empty($param['surgeryEndDate'])) { ?>
												<?php echo date('g:i A', strtotime($param['surgeryEndDate'])); ?>
												<?php echo date('d-m-y', strtotime($param['surgeryEndDate'])); ?>
											<?php } ?>
										</td>
									</tr>
								...
								if ($this->session->user_role == 4) {
									...
									$this->db->join('setup_doctor', 'setup_doctor.shortkey = department.slug');
									// $this->db->where('department.email', $email);
									$this->db->where('setup_doctor.parent', 1);
									$this->db->where('department.type', 'doctor');
									$query = $this->db->get();
									...
									$sresult = $this->doctorsfeedback_model->setup_result('setup_doctor');
								}
													...
													<?php if ($param['comment'][$k]) { ?>
														<ul class="feedback">
															<li id="feedback" style="overflow: clip; word-break: break-all;">
																<?php if (is_mobile() === true) { ?>
																	<h6><?php echo 'Doctor Remarks:'; ?></h6>
																<?php } else { ?>
																	<h5><?php echo 'Doctor Remarks:';  ?></h5>
																<?php } ?>
																<p>
									...
									<ul class="feedback">
										...
										<?php if ($this->session->user_role <= 3) { ?>
											<?php if ((($param['location'] == 1)) || ($param['specificservice'] == 1) || ($param['referred'] == 1) || ($param['friend'] == 1) || ($param['previous'] == 1) ||  ($param['docAvailability'] == 1) || ($param['companyRecommend'] == 1) || ($param['otherReason'] == 1)) { ?>
												...
												<p>
													<?php if ($param['location'] == 1) { ?><li id="feedback">Quality patient care</li><?php } ?>
													<?php if ($param['specificservice'] == 1) { ?><li id="feedback">Advanced technology</li><?php } ?>
													<?php if ($param['referred'] == 1) { ?><li id="feedback">Efficient support staff</li><?php } ?>
													<?php if ($param['friend'] == 1) { ?><li id="feedback">Collaborative environment</li><?php } ?>
													<?php if ($param['previous'] == 1) { ?><li id="feedback">Well-maintained facilities</li><?php } ?>
													<?php if ($param['docAvailability'] == 1) { ?><li id="feedback">Strong leadership</li><?php } ?>
													<?php if ($param['companyRecommend'] == 1) { ?><li id="feedback">Professional development</li><?php } ?>
													<?php if ($param['otherReason'] == 1) { ?><li id="feedback">Good communication</li><?php } ?>
												</p>
											<?php } ?>
