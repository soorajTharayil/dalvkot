<div class="content">
	...
	$ip_feedbacks_count = $this->grievance_model->patient_and_feedback($table_patients, $table_feedback, $sorttime, $setup);
	$ip_psat = $this->grievance_model->psat_analytics($table_patients, $table_feedback, $table_tickets, $asc);
	?>
	<!-- Metric Boxes-->
	<div class="row">
			...
			<div class="panel panel-bd">
				<div class="panel-body" style="height: 100px;">
					<div class="statistic-box">
						...
						<div class="small"><?php echo lang_loader('sg','sg_total_feedbacks'); ?> <a href="javascript:void()" data-toggle="tooltip" title="<?php echo $totalfeedback_info_tooltip; ?>"><i class="fa fa-info-circle" aria-hidden="true"></i></i></a></div>
						<div class="icon">
							<i class="fa fa-comments-o"></i>
						</div>
					</div>
					<a href="<?php echo $ip_link_feedback_report; ?>" style="float: right;    margin-top: -9px;"><?php echo lang_loader('sg','sg_view_list'); ?></a>
	...
	<div class="panel panel-default">
		...
		<div class="panel-body">
			...
			<table class="sgticketanalisys table table-striped table-bordered table-hover" cellspacing="0" width="100%">
				<thead>
					<th><?php echo lang_loader('sg','sg_slno'); ?></th>
					<th><?php echo lang_loader('sg','sg_department'); ?></th>
					<!-- <th>Percentage</th> -->
					<th><?php echo lang_loader('sg','sg_s_total_grievances'); ?></th>
					<th><?php echo lang_loader('sg','sg_s_open_grievances'); ?></th>
					<!-- change here according to helper -->
					<?php if (ticket_addressal('grievance_addressal') === true) { ?>
						<th><?php echo lang_loader('sg','sg_s_addressed_grievances'); ?></th>
					<?php } ?>
					<th><?php echo lang_loader('sg','sg_s_closed_grievances'); ?></th>
					<?php if ($this->uri->segment(2) == "ticket_resolution_rate") { ?>
						<th><?php echo lang_loader('sg','sg_resolution_rate'); ?></th>
					<?php } ?>
					<?php if ($this->uri->segment(2) == "average_resolution_time") { ?>
						<th><?php echo lang_loader('sg','sg_avg_resolution_time'); ?></th>
					<?php } ?>
				</thead>
				<?php $sl = 1;
				$i = 0; ?>
