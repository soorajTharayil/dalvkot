<div class="content">
	...
	$all_feedback = $this->grievance_model->patient_and_feedback($table_patients, $table_feedback_int, $sorttime, $setup_int);



	$interim_tickets_count = $this->grievance_model->feedback_and_ticket($table_feedback_int, $table_tickets_int, $sorttime);

	$open_tickets_int = $this->grievance_model->tickets_feeds($table_feedback_int, $table_tickets_int, $sorttime, $open);
	$closed_tickets_int = $this->grievance_model->tickets_feeds($table_feedback_int, $table_tickets_int, $sorttime, $closed);
	$addressed_tickets_int = $this->grievance_model->tickets_feeds($table_feedback_int, $table_tickets_int, $sorttime, $addressed);
								...
								if (($param->other != '' && $param->other != NULL)) { ?>
									...
									<div class="inbox-item">
										...
										<?php if ($param->suggestionText != '' && $param->suggestionText != NULL) { ?>
											<p class="inbox-item-text"><i class="fa fa-user-plus" aria-hidden="true"></i>&nbsp;<b><?php echo lang_loader('sg','sg_other_comment'); ?></b>:
