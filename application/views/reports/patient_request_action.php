<div class="content">
   ...
   $this->db->select('tickets_service.*');
   $this->db->from('tickets_service');

   $this->db->join('bf_feedback_service', 'tickets_service.feedbackid = bf_feedback_service.id', 'inner');
   $this->db->join('bf_patients_service', 'bf_patients_service.patient_id = bf_feedback_service.patientid', 'inner');
   ...
   $this->db->where('tickets_service.last_modified <=', $fdate);
   $this->db->where('tickets_service.last_modified >=', $tdate);
   $this->db->where('tickets_service.status', 'Closed');
                  ...
                  <?php foreach ($departments as $dep) { ?>
                     <div class="panel-heading no-print">
                        <div class=" text-center">
                           <?php
                           $departments = $this->ticketspat_model->read_by_id($dep->id);
                           $department = $departments[0];
                           ?>
                           <h3>Request ID: PSR-<?php echo $department->id; ?></h3>
                        </div>
                     </div>
                     <div class="col-md-8">
                        <div class="panel-body">
                           <table class=" table table-striped table-bordered dataTable no-footer dtr-inline ">
                              ...
                              <tr>
                                 ...
                                 <td>
                                    <b><?php echo $department->patinet->name; ?> (<a href="<?php echo base_url('report/patient_request_details'); ?>?patientid=<?php echo  $department->patinet->patient_id; ?>"><?php echo  $department->patinet->patient_id; ?></a>)
                                 </td>
                              </tr>
                               <tr>
                                 <td>Ward</td>
