...
class Analytics_asset extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->table_feedback = 'bf_feedback_asset_creation';
        $this->table_patients = 'bf_patients';
        $this->sorttime = 'asc';
        $this->setup = 'setup_int';
        $this->asc = 'asc';
        $this->desc = 'desc';
        $this->table_tickets = 'tickets_asset';
        $this->open = 'Open';
        $this->closed = 'Closed';
    ...
    public function tickets_recived_by_department()
    {
        ...
        $department = $this->asset_model->get_departmentint($this->type);

        $all_tickes = $this->asset_model->get_tickets($this->table_feedback, $this->table_tickets);
        $report = array();
        $response = array();
    ...
    public function tickets_recived_by_floor_set()
    {
        $selected_floor = isset($_GET['settype']) ? $_GET['settype'] : '';

        // Get the row for the selected floor
        $floor_row = $this->db->select('bed_no')  // ✅ FIXED: 'bed_no' is the correct column name
            ->from('bf_ward_esr')
            ->where('title', $selected_floor)
            ->get()
            ->row();
        ...
        // Decode the JSON array from 'bed_no'
        $bednos = json_decode($floor_row->bed_no, true); // ✅ FIXED: decode from correct column
        ...
        // Count matches per site (bedno) under the selected floor
        foreach ($bednos as $bedno) {
            ...
            foreach ($feedbacks as $feedback) {
                ...
                if (
                    isset($dataset['locationsite'], $dataset['bedno']) &&
                    trim($dataset['locationsite']) === trim($selected_floor) &&
                    trim($dataset['bedno']) === trim($bedno)
                ) {
                    $count++;
                }
            }
