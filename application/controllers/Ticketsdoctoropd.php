...
class Ticketsdoctoropd extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['title'] = 'OPD DOCTOR- OPEN TICKETS LIST';
		#-------------------------------#
		$data['departments'] = $this->ticketsopddoctor_model->read();

		$data['content'] = $this->load->view('doctoropd/opentickets', $data, true);
	...
	public function alltickets()
	{
		...
		$data['title'] = 'OPD DOCTOR- ALL TICKETS LIST';
		#-------------------------------#
		$data['departments'] = $this->ticketsopddoctor_model->alltickets();
		$data['content'] = $this->load->view('doctoropd/alltickets', $data, true);
		$this->load->view('layout/main_wrapper', $data);
		$this->session->set_userdata('referred_from', NULL);
	}
	...
	public function addressedtickets()
	{
		...
		$data['title'] = 'OPD DOCTOR- ADDRESSED TICKETS LIST';
		#-------------------------------#
		$data['departments'] = $this->ticketsopddoctor_model->addressedtickets();

		$data['content'] = $this->load->view('doctoropd/addressedtickets', $data, true);
		$this->load->view('layout/main_wrapper', $data);
		$this->session->set_userdata('referred_from', NULL);
	}
	...
	public function ticket_track()
	{
		...
		$data['content'] = $this->load->view('doctoropd/ticket', $data, true);
		$this->load->view('layout/main_wrapper', $data);
	}
	...
	public function ticket_close()
	{
		...
		$data['title'] = 'OPD DOCTOR- CLOSED TICKETS LIST';
		$data['departments'] = $this->ticketsopddoctor_model->read_close();
		$data['content'] = $this->load->view('doctoropd/ticket_close', $data, true);
	...
	public function track_close()
	{
		...
		$data['title'] = 'OPD DOCTOR- CLOSED TICKET DETAILS';
		$data['departments'] = $this->ticketsopddoctor_model->read_by_id($this->uri->segment(3));
		$data['content'] = $this->load->view('doctoropd/track_close', $data, true);
		$this->load->view('layout/main_wrapper', $data);
		$this->session->set_userdata('referred_from', NULL);
	}
	...
	public function create($dprt_id = null)
	{
		...
		if ($this->input->post('deparment') != 0) {
			...
			$this->db->update('tickets_doctor_opd', $updatedepartment);
			...
			$this->db->insert('ticket_doctor_opd_message	', $dataset);
			$curl = curl_init();
			curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
			curl_setopt($curl, CURLOPT_URL, base_url() . 'api/curl.php');
			curl_exec($curl);
			redirect('doctoropd/opentickets');
		} else {
			...
			if ($this->input->post('status') == 'Closed') {
				...
				$this->db->update('tickets_doctor_opd', $updatedepartmen);
				//}
				...
				$this->db->insert('ticket_doctor_opd_message	', $dataset);
				// redirect('doctoropd/closedtickets');
				...
				$this->db->insert('ticket_doctor_opd_message	', $dataset);
				redirect('doctoropd/addressedtickets');
			} else {
				$action = 'Comment';
				$message = $this->session->userdata['fullname'];
				$dataset = array(
		...
		redirect('doctoropd/track/' . $this->input->post('id'));
	}
}
