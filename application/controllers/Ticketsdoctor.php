...
class Ticketsdoctor extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['title'] = 'OT DOCTOR- OPEN TICKETS LIST';
		#-------------------------------#
		$data['departments'] = $this->ticketsdoctor_model->read();

		$data['content'] = $this->load->view('doctor/opentickets', $data, true);
	...
	public function alltickets()
	{
		...
		$data['title'] = 'OT DOCTOR- ALL TICKETS LIST';
		#-------------------------------#
		$data['departments'] = $this->ticketsdoctor_model->alltickets();
		$data['content'] = $this->load->view('doctor/alltickets', $data, true);
		$this->load->view('layout/main_wrapper', $data);
		$this->session->set_userdata('referred_from', NULL);
	}
	...
	public function addressedtickets()
	{
		...
		$data['title'] = 'OT DOCTOR- ADDRESSED TICKETS LIST';
		#-------------------------------#
		$data['departments'] = $this->ticketsdoctor_model->addressedtickets();

		$data['content'] = $this->load->view('doctor/addressedtickets', $data, true);
		$this->load->view('layout/main_wrapper', $data);
		$this->session->set_userdata('referred_from', NULL);
	}
	...
	public function ticket_track()
	{
		...
		$data['content'] = $this->load->view('doctor/ticket', $data, true);
		$this->load->view('layout/main_wrapper', $data);
	}
	...
	public function ticket_close()
	{
		...
		$data['title'] = 'OT DOCTOR- CLOSED TICKETS LIST';
		$data['departments'] = $this->ticketsdoctor_model->read_close();
		$data['content'] = $this->load->view('doctor/ticket_close', $data, true);
	...
	public function track_close()
	{
		...
		$data['title'] = 'OT DOCTOR- CLOSED TICKET DETAILS';
		$data['departments'] = $this->ticketsdoctor_model->read_by_id($this->uri->segment(3));
		$data['content'] = $this->load->view('doctor/track_close', $data, true);
		$this->load->view('layout/main_wrapper', $data);
		$this->session->set_userdata('referred_from', NULL);
	}
	...
	public function create($dprt_id = null)
	{
		...
		if ($this->input->post('deparment') != 0) {
			...
			$this->db->update('tickets_doctor', $updatedepartment);
			...
			$this->db->insert('ticket_doctor_message', $dataset);
			$curl = curl_init();
			curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
			curl_setopt($curl, CURLOPT_URL, base_url() . 'api/curl.php');
			curl_exec($curl);
			redirect('doctor/opentickets');
		} else {
			...
			if ($this->input->post('status') == 'Closed') {
				...
				$this->db->update('tickets_doctor', $updatedepartmen);
				//}
				...
				$this->db->insert('ticket_doctor_message', $dataset);
				redirect('doctor/addressedtickets');
			} else {
				$action = 'Comment';
				$message = $this->session->userdata['fullname'];
				$dataset = array(
		...
		redirect('doctor/track/' . $this->input->post('id'));
	}
}
