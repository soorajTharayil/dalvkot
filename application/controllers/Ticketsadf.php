...
class Ticketsadf extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['content'] = $this->load->view('admissionfeedback/ticket', $data, true);
	...
	public function addressedtickets()
	{
		...
		$data['content'] = $this->load->view('admissionfeedback/addressedtickets', $data, true);
		$this->load->view('layout/main_wrapper', $data);
		$this->session->set_userdata('referred_from', NULL);
	}
	...
	public function alltickets()
	{
		...
		$data['content'] = $this->load->view('admissionfeedback/alltickets', $data, true);
		$this->load->view('layout/main_wrapper', $data);
		$this->session->set_userdata('referred_from', NULL);
	}
	...
	public function ticket_track()
	{
		...
		$data['content'] = $this->load->view('admissionfeedback/ticket_track', $data, true);
		$this->load->view('layout/main_wrapper', $data);
	}
	...
	public function ticket_close()
	{
		...
		$data['content'] = $this->load->view('admissionfeedback/ticket_close', $data, true);
	...
	public function track_close()
	{
		...
		$data['departments'] = $this->ticketsadf_model->read_by_id($this->uri->segment(3));
		$data['content'] = $this->load->view('admissionfeedback/track_close', $data, true);
		$this->load->view('layout/main_wrapper', $data);
		$this->session->set_userdata('referred_from', NULL);
	}
	...
	public function create($dprt_id = null)
	{
		...
		if ($this->input->post('deparment') != 0) {
			...
			$this->db->update('tickets_adf', $updatedepartment);
			...
			$this->db->insert('ticket_message_adf', $dataset);
			$curl = curl_init();
			curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
			curl_setopt($curl, CURLOPT_URL, base_url() . 'api/curl.php');
			curl_exec($curl);
			redirect('admissionfeedback/opentickets');
		} else {
			if ($this->input->post('status') == 'Closed') {
				...
				$this->db->update('tickets_adf', $updatedepartmen);
				//}
				if (close_comment('adf_close_comment') === true) {
				...
				$this->db->insert('ticket_message_adf', $dataset);
				// redirect('admissionfeedback/closedtickets');
				...
				$this->db->insert('ticket_message_adf', $dataset);
				redirect('admissionfeedback/addressedtickets');
			} else {
				$action = 'Comment';
				$message = $this->session->userdata['fullname'];
				$dataset = array(
		...
		redirect('admissionfeedback/track/' . $this->input->post('id'));
	}




	public function editcapa($dprt_id = null)
	{
		$result = $this->db->select("*")
			->from('tickets_adf')
			->where('id', $this->input->post('id'))
		...
		$resultss = $this->db->select("*")
			...
			->result();
		//print_r($resultss); exit;
		$action = 'Edited by ' . $this->session->userdata['fullname'];
		$message = $this->session->userdata['fullname'];
		//foreach($resultss as $tickets){

		$updatedepartmen = array('status' => 'closed');
		...
		$this->db->update('ticket_message_adf', $dataset);
		redirect('admissionfeedback/editcapa/' . $this->input->post('id'));
	}
}
