...
class Ticketsint extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['content'] = $this->load->view('pc/ticket', $data, true);
	...
	public function alltickets()
	{
		...
		$data['content'] = $this->load->view('pc/alltickets', $data, true);
		$this->load->view('layout/main_wrapper', $data);
		$this->session->set_userdata('referred_from', NULL);
	}
	...
	public function addressedtickets()
	{
		...
		$data['content'] = $this->load->view('pc/addressedtickets', $data, true);
		$this->load->view('layout/main_wrapper', $data);
		$this->session->set_userdata('referred_from', NULL);
	}
	...
	public function ticket_track()
	{
		...
		$data['content'] = $this->load->view('pc/track', $data, true);
		$this->load->view('layout/main_wrapper', $data);
	}
	...
	public function ticket_close()
	{
		...
		$data['content'] = $this->load->view('pc/ticket_close', $data, true);
	...
	public function track_close()
	{
		...
		$data['departments'] = $this->ticketsint_model->read_by_id($this->uri->segment(3));
		$data['content'] = $this->load->view('pc/track_close', $data, true);
		$this->load->view('layout/main_wrapper', $data);
		$this->session->set_userdata('referred_from', NULL);
	}
	...
	public function create($dprt_id = null)
	{
		...
		if ($this->input->post('deparment') != 0) {
			...
			if ($this->input->post('status') == 'Closed') {
				...
				$this->db->update('tickets_int', $updatedepartmen);
				//}
				if (close_comment('pc_close_comment') === true) {
				...
				$this->db->insert('ticket_int_message', $dataset);
				//redirect('pc/closedtickets');
				if ($this->input->post('status_patient_verified') && $this->input->post('status_patient_verified') == 'Verified') {

					redirect('track/pc/' . $this->input->post('id'));

				} else {
					redirect('pc/alltickets');
				}
				...
				if ($this->input->post('status_patient') && $this->input->post('status_patient') == 'Patient') {
					...
					$action = 'Reopened by the patient (' . $this->input->post('patient_reopen_name') . ')';
		...
		redirect('pc/track/' . $this->input->post('id'));
	}
}
