...
class Message extends CI_Controller
{
	...
	public function inweeklyreport()
	{
		...
		$message1 = 'Weekly IP feedbacks report: ' . $d1 . ' to ' . $d2 . '%0a%0aTotal feedbacks:' . count($feedbacktaken) . '%0aSatisfied Patients:' . (count($feedbacktaken) - $below3) . '%0aUnsatisfied Patients:' . $below3 . '%0aSatisfaction Index:' . round((count($feedbacktaken) - $below3) / (count($feedbacktaken)) * 100) . '%25%0aNPS Score:' . round((($promoter - $depromoter) / ($depromoter + $promoter + $passive)) * 100) . '%25';


		$TEMPID = '1607100000000084987';
		$user = $this->get_user_by_sms_activity('IP-WEEKLY-FEEDBACKS-REPORT-SMS');
