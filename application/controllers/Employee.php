...
class Employee extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['content'] = $this->load->view('employees_esr/patient', $data, true);
	...
	// {
	// 	$data['title'] = 'Interim Employee List';
	// 	$data['patients'] = $this->employee_model->read_interim();
	// 	$data['content'] = $this->load->view('employees_esr/interim',$data,true);
	...
	// 	$data['patients'] = $this->employee_model->readdischarged();
	// 	$data['content'] = $this->load->view('employees_esr/patient_discharge_list', $data, true);
	// 	$this->load->view('layout/main_wrapper', $data);
	// }
	...
	public function create()
	{
		...
		if ($this->input->post('id') == null) { //create a patient
			...
			$query = $this->db->get('healthcare_employees');
			if ($query->result()) {
			...
			// $query = $this->db->get('healthcare_employees');
			// if ($query->result()) {
			// 	$this->session->set_flashdata('exception', 'Duplicate Email ID');
			// 	//redirect('employee/create');
			// 	$error = true;
			// }
			...
			$query = $this->db->get('healthcare_employees');
			if ($query->result()) {
				$this->session->set_flashdata('exception', 'Duplicate Employee ID');
				//redirect('employee/create');
				$error = true;
			}
		...
		if ($this->form_validation->run() === true && $error == false) {
			...
			$data['content'] = $this->load->view('employees_esr/patient_form_create', $data, true);
			$this->load->view('layout/main_wrapper', $data);
		}
	}
	...
	public function profile($patient_id = null)
	{
		...
		#-------------------------------#
		$data['profile'] = $this->employee_model->read_by_id($patient_id);
	...
	{
		$data['title'] = 'EDIT EMPLOYEE INFO';
		#-------------------------------#
		$data['patient'] = $this->employee_model->read_by_id($patient_id);
		// if ($data['patient']->discharged_date != 0) {
		// 	$data['patient']->discharged_date =  date('Y-m-d H:i:s', strtotime($data['patient']->discharged_date));
		// }
		$data['content'] = $this->load->view('employees_esr/edit_patient', $data, true);
		$this->load->view('layout/main_wrapper', $data);
	}
    ...
    */
	...
	public function document_form()
	{
		...
		if ($this->form_validation->run() === true) {
			...
			redirect('employee/document_form');
		} else {
