...
class Tickets_pdf extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['title'] = 'PDF- OPEN TICKETS LIST';
		#-------------------------------#
		$data['departments'] = $this->tickets_model->read();

		$data['content'] = $this->load->view('post/ticket', $data, true);
	...
	public function addressedtickets()
	{
		...
		$data['title'] = 'PDF- ADDRESSED TICKETS LIST';
		#-------------------------------#
		$data['departments'] = $this->tickets_model->addressedtickets();

		$data['content'] = $this->load->view('post/addressedtickets', $data, true);
		$this->load->view('layout/main_wrapper', $data);
		$this->session->set_userdata('referred_from', NULL);
	}
	...
	public function alltickets()
	{
		...
		$data['title'] = 'PDF- ALL TICKETS LIST';
		#-------------------------------#
		$data['departments'] = $this->tickets_model->alltickets();
		$data['content'] = $this->load->view('post/alltickets', $data, true);
		$this->load->view('layout/main_wrapper', $data);
		$this->session->set_userdata('referred_from', NULL);
	}
	...
	public function ticket_track()
	{
		...
		$data['content'] = $this->load->view('post/ticket_track', $data, true);
		$this->load->view('layout/main_wrapper', $data);
	}
	...
	public function ticket_close()
	{
		...
		$data['title'] = 'PDF- CLOSED TICKETS LIST';
		$data['departments'] = $this->tickets_model->read_close();
		$data['content'] = $this->load->view('post/ticket_close', $data, true);
	...
	public function track_close()
	{
		...
		$data['title'] = 'PDF- CLOSED TICKET DETAILS';
		$data['departments'] = $this->tickets_model->read_by_id($this->uri->segment(3));
		$data['content'] = $this->load->view('post/track_close', $data, true);
		$this->load->view('layout/main_wrapper', $data);
		$this->session->set_userdata('referred_from', NULL);
	}
	...
	public function create($dprt_id = null)
	{
		...
		if ($this->input->post('deparment') != 0) {
			...
			$this->db->update('tickets_pdf', $updatedepartment);
			...
			$this->db->insert('ticket_pdf_message', $dataset);
			$curl = curl_init();
			curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
			curl_setopt($curl, CURLOPT_URL, base_url() . 'api/curl.php');
			curl_exec($curl);
			redirect('post/opentickets');
		} else {
			if ($this->input->post('status') == 'Closed') {
				...
				$this->db->update('tickets_pdf', $updatedepartmen);
				//}
				...
				$this->db->insert('ticket_pdf_message', $dataset);
				redirect('post/addressedtickets');
			} else {
				$action = 'Comment';
				$message = $this->session->userdata['fullname'];
				$dataset = array(
		...
		redirect('post/track/' . $this->input->post('id'));
	}
}
