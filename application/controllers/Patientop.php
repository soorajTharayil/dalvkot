...
class Patientop extends CI_Controller
{
	...
	{
		$data['title'] = 'ACTIVE OUTPATIENT LIST';
		$data['patients'] = $this->patientop_model->read();
		$data['content'] = $this->load->view('patientop/patient', $data, true);
	...
	{
		$data['title'] = 'FORMER OUTPATIENT LIST';
		$data['patients'] = $this->patientop_model->readdischarged();
		$data['content'] = $this->load->view('patientop/patient_discharge_list', $data, true);
		$this->load->view('layout/main_wrapper', $data);
	}
	...
	public function create()
	{
		...
		$error = false;
		$data['title'] = 'OUTPATIENT ADMISSION ';
		...
		if ($this->input->post('id') == null) { //create a patient
			...
			$query = $this->db->get('patients_from_admission_op');
			if ($query->result()) {
				$this->session->set_flashdata('exception', 'Duplicate Patient ID');
				//redirect('patientop/create');
				$error = true;
			}
		...
		if ($this->form_validation->run() === true && $error == false) {
			...
			if (empty($postData['id'])) {
				...
				if ($this->patientop_model->create($postData)) {
				...
				if ($error == false) {
					if ($this->patientop_model->update($postData)) {
						#set success message
			...
			$data['content'] = $this->load->view('patientop/patient_form_create', $data, true);
			$this->load->view('layout/main_wrapper', $data);
		}
	}
	...
	public function movetoinpatient()
	{
		...
		$query = $this->db->update('patients_from_admission_op', $data);
		$result = $query->result();
	}
	...
	public function profile($patient_id = null)
	{
		...
		#-------------------------------#
		$data['profile'] = $this->patientop_model->read_by_id($patient_id);
	...
	public function summary($patient_id = null)
	{
		...
		$data['content'] = $this->load->view('patientop/welcome', $data, true);
		$this->load->view('layout/main_wrapper', $data);
	}
	...
	// 	// }
		...
		if ($this->patientop_model->set_discharge($patient_id)) {
			#set success message
	...
	// 	// }
		...
		if ($this->patientop_model->dama($patient_id)) {
			#set success message
	...
	{
		if ($this->patientop_model->delete($patient_id)) {
			#set success message
    ...
    */
	...
	public function document_form()
	{
		...
		if ($this->form_validation->run() === true) {
			...
			redirect('patientop/document_form');
		} else {
