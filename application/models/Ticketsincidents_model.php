...
class Ticketsincidents_model extends CI_Model
{
	...
	public function addressedtickets()
	{
		...
		foreach ($result as $row) {
			...
			if (($pos !== false && count($this->session->userdata['department_access'][$department[0]->type]) > 0) || $row->assign_to == $this->session->userdata['user_id'] || $row->reassign_to == $this->session->userdata['user_id']) {
