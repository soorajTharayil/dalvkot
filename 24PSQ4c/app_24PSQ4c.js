...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window) {
	...
	$scope.setupapplication = function () {
		...
		//alert(id);
		$http.get($rootScope.baseurl_main + '/api_24PSQ4c.php?patientid=' + id + '&month=' + selectedMonths + '&year=' + selectedYears, { timeout: 20000 }).then(function (responsedata) {
			$scope.feedback.initial_assessment_hr = responsedata.data.totalHoursGeneral;
			$scope.feedback.initial_assessment_min = responsedata.data.totalMinutesGeneral;
			$scope.feedback.initial_assessment_sec = responsedata.data.totalSecondsGeneral;
			$scope.feedback.total_admission = responsedata.data.totalAdmissionGeneral;

			$scope.feedback.initial_assessment_hr2 = responsedata.data.totalHoursInsurance;
			$scope.feedback.initial_assessment_min2 = responsedata.data.totalMinutesInsurance;
			$scope.feedback.initial_assessment_sec2 = responsedata.data.totalSecondsInsurance;
			$scope.feedback.total_admission2 = responsedata.data.totalAdmissionInsurance;

			$scope.feedback.initial_assessment_hr3 = responsedata.data.totalHoursCorporate;
			$scope.feedback.initial_assessment_min3 = responsedata.data.totalMinutesCorporate;
			$scope.feedback.initial_assessment_sec3 = responsedata.data.totalSecondsCorporate;
			$scope.feedback.total_admission3 = responsedata.data.totalAdmissionCorporate;
	...
	$scope.calculateTimeFormat = function () {
		...
		// Get total admission
		var totalPatientsDischarged = parseInt(document.getElementById('formula_para2').value);
		...
		var averageSeconds = Math.floor(assessmentSeconds / totalPatientsDischarged);
		...
		if (excessSeconds <= 0) {
			...
			$scope.feedback.excessTime = `${excessHours}:${('0' + excessMinutes).slice(-2)}:${('0' + excessRemainingFinalSeconds).slice(-2)}`;
	...
	$scope.calculateTimeFormat2 = function () {
		...
		var hoursInput = parseInt(document.getElementById('formula_para1_hr2').value || 0);
		var minutesInput = parseInt(document.getElementById('formula_para1_min2').value || 0);
		var secondsInput = parseInt(document.getElementById('formula_para1_sec2').value || 0);

		// Convert into total seconds
		var assessmentSeconds = (hoursInput * 3600) + (minutesInput * 60) + secondsInput;

		// Get total admission
		var totalPatientsDischarged = parseInt(document.getElementById('formula_para2_i').value);
		...
		// Format the result to "hr:min:sec"
		$scope.calculatedResult2 = `${avgHours}:${('0' + avgMinutes).slice(-2)}:${('0' + avgSeconds).slice(-2)}`;
		...
		if (excessSeconds <= 0) {
			...
			$scope.feedback.excessTime2 = `${excessHours}:${('0' + excessMinutes).slice(-2)}:${('0' + excessRemainingFinalSeconds).slice(-2)}`;
	...
	$scope.calculateTimeFormat3 = function () {
		...
		var hoursInput = parseInt(document.getElementById('formula_para1_hr3').value || 0);
		var minutesInput = parseInt(document.getElementById('formula_para1_min3').value || 0);
		var secondsInput = parseInt(document.getElementById('formula_para1_sec3').value || 0);

		// Convert into total seconds
		var assessmentSeconds = (hoursInput * 3600) + (minutesInput * 60) + secondsInput;

		// Get total admission
		var totalPatientsDischarged = parseInt(document.getElementById('formula_para2_c').value);
		...
		// Format the result to "hr:min:sec"
		$scope.calculatedResult3 = `${avgHours}:${('0' + avgMinutes).slice(-2)}:${('0' + avgSeconds).slice(-2)}`;
		...
		if (excessSeconds <= 0) {
			...
			$scope.feedback.excessTime3 = `${excessHours}:${('0' + excessMinutes).slice(-2)}:${('0' + excessRemainingFinalSeconds).slice(-2)}`;
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_24PSQ4c')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/savepatientfeedback_kpi24.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
