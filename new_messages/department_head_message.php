        ...
        foreach ($user_list as $user_row) {
            ...
            if (is_null($floor_wards) || empty($floor_wards) || in_array($patient_ward, $floor_wards)) {
                ...
                if (!empty($users_whatsapp)) {
                 ...
                 VALUES ('91$number', 'ITATONE POINT CONSULTING LLP 7345', 'staffsms_alert_inpatientcomplaint', '" . json_encode([$hospitalname, $patient_name, $patient_uhid, $patient_ward, $patient_bedno, $Concern_Category, $Concern_Area, $department_head_link_whatsapp]) . "', 
        ...
        foreach ($user_list as $row) {
            ...
            if (is_null($floor_wards) || empty($floor_wards) || in_array($patient_ward, $floor_wards)) {
                ...
                if (!empty($users_whatsapp)) {
                    ...
                    $insert_notification_query = "INSERT INTO notifications_whatsapp (destination, userName, campaignName, templateParams, source, media, buttons, carouselCards, location, paramsFallbackValue, status,meta,uuid) 
             VALUES ('91$number', 'ITATONE POINT CONSULTING LLP 7345', 'staffsms_alert_inpatient_negativeexperiences', '" . json_encode([$hospitalname, $patient_name, $patient_uhid, $patient_ward, $patient_bedno, $Concern_Category, $Concern_Area, $department_head_link_whatsapp]) . "', 
             'new-landing-page form', '{}', '[]', '[]', '{}', '" . json_encode(["FirstName" => "user"]) . "', 'pending','" . mysqli_real_escape_string($con, json_encode($meta_data)) . "','" . $uuid . "')";
