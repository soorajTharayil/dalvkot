...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window) {
	...
	$scope.setupapplication = function () {
		...
		//alert(id);
		$http.get($rootScope.baseurl_main + '/api_17PSQ3b.php?patientid=' + id + '&month=' + selectedMonths + '&year=' + selectedYears, { timeout: 20000 }).then(function (responsedata) {
			$scope.feedback.initial_assessment_hr = responsedata.data.hygieneCount;
			$scope.feedback.total_admission = responsedata.data.opportunitiesCount;
	...
	$scope.calculateHandHygieneComplianceRate = function () {
		...
		var handHygieneActions = parseInt(document.getElementById('formula_para1').value);

		// Get the total number of hand hygiene opportunities
		var handHygieneOpportunities = parseInt(document.getElementById('formula_para2').value);
		...
		if (handHygieneActions === 0 && handHygieneOpportunities === 0) {
			...
			$scope.calculatedResult = ((handHygieneActions / handHygieneOpportunities) * 100).toFixed(2);
		}

		// Store the result in the feedback object for further use
		$scope.feedback.calculatedResult = $scope.calculatedResult;

		console.log("Calculated Hand Hygiene Compliance Rate", $scope.calculatedResult);
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_17PSQ3b')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/savepatientfeedback_kpi17.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
