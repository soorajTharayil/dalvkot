...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window) {
    ...
    $scope.calculateDepreciation = function () {
        ...
        // Overall asset value as of today
        ...
        while (fyStart <= currentDate) {
            ...
            if (fyYear >= currentFYStart - 2) {
                let periodStart = new Date(Math.max(fyStart, installDate));
                let periodEnd;

                if (fyYear === currentFYStart) {
                    // For current FY: depreciate till today
                    periodEnd = new Date(currentDate);
                } else {
                    // For past FYs: till March 31
                    periodEnd = new Date(fyYear + 1, 2, 31);
                }
