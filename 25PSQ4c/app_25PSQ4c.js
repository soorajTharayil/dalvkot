...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window) {
	...
	$scope.setupapplication = function () {
		...
		//alert(id);
		$http.get($rootScope.baseurl_main + '/api_25PSQ4c.php?patientid=' + id + '&month=' + selectedMonths + '&year=' + selectedYears, { timeout: 20000 }).then(function (responsedata) {
	...
	$scope.calculateIncompleteConsentRate = function () {
		...
		var incompleteConsentRecords = parseInt(document.getElementById('formula_para1').value);

		// Get the number of discharges and deaths
		var dischargesAndDeaths = parseInt(document.getElementById('formula_para2').value);
		...
		if (incompleteConsentRecords === 0 && dischargesAndDeaths === 0) {
			...
			$scope.calculatedResult = ((incompleteConsentRecords / dischargesAndDeaths) * 100).toFixed(2);
		}

		// Store the result in the feedback object for further use
		$scope.feedback.calculatedResult = $scope.calculatedResult;

		console.log("Calculated Incomplete Consent Rate", $scope.calculatedResult);
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_25PSQ4c')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/savepatientfeedback_kpi25.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
