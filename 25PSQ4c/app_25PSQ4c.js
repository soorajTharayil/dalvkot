...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateIncompleteConsentRate = function () {
      ...
      var incompleteConsentRecords =
        document.getElementById("formula_para1").value;

      // Get the number of discharges and deaths
      var dischargesAndDeaths = document.getElementById("formula_para2").value;

      // Alert if inputs are empty
      if (incompleteConsentRecords === "") {
        alert(
          "Enter the number of medical records having incomplete and/or improper consent."
        );
        return;
      }
      if (dischargesAndDeaths === "") {
        alert("Enter the number of discharges and deaths.");
        return;
      }

      // Convert to integers after empty check
      incompleteConsentRecords = parseInt(incompleteConsentRecords);
      dischargesAndDeaths = parseInt(dischargesAndDeaths);

      // Validate negative numbers
      if (incompleteConsentRecords < 0) {
        alert(
          "Enter the number of medical records having incomplete and/or improper consent."
        );
        return;
      ...
      if (incompleteConsentRecords === 0 && dischargesAndDeaths === 0) {
        ...
        $scope.calculatedResult = (
          (incompleteConsentRecords / (dischargesAndDeaths || 1)) *
          100
        ).toFixed(2);
      }
