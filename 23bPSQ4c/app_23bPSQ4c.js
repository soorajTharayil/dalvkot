...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateTimeFormat = function () {
      // Check numerator fields individually for empty (not zero)
      ...
      if (hrField === "" && minField === "" && secField === "") {
        alert("Please enter numerator values");
        return;
      }

      // Check denominator empty (not zero)
      var denomField = document.getElementById("formula_para2").value;
      if (denomField === "") {
        alert("Please enter denominator value");
        return;
      }

      // Continue original logic EXACTLY as you wrote
      ...
      // Allow 0 but prevent negative
      if (isNaN(totalPatientInDiagnostics)) {
