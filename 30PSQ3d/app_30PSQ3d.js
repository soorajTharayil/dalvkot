...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateNeedleStickInjuryRate = function () {
      ...
      var parenteralExposures = document.getElementById("formula_para1").value;

      // Get the number of in-patient days
      var inpatientDays = document.getElementById("formula_para2").value;

      // Alert if inputs are empty
      if (parenteralExposures === "") {
      ...
      // Convert to integers
      parenteralExposures = parseInt(parenteralExposures);
      inpatientDays = parseInt(inpatientDays);

      // Validate negative numbers
      if (parenteralExposures < 0) {
        alert("Enter the number of parenteral exposures.");
        return;
      }
      if (inpatientDays < 0) {
      ...
      if (parenteralExposures === 0 && inpatientDays === 0) {
        ...
        // Calculate the incidence per 1000 in-patient days safely
        $scope.calculatedResult = (
          (parenteralExposures / (inpatientDays || 1)) *
          1000
        ).toFixed(2);
      }
