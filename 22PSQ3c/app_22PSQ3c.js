...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateTimeFormat = function () {
      ...
      if (isNaN(totalOutPatients)) {
        alert("Please enter the above value to calculate");
        return;
      }

      // Special case → 0 numerator and 0 denominator
      if (assessmentSeconds === 0 && totalOutPatients === 0) {
      ...
      if (totalOutPatients === 0) {
        $scope.calculatedResult = assessmentSeconds + ":00:00";
        $scope.feedback.calculatedResult = $scope.calculatedResult;
        $scope.valuesEdited = false;
        return;
      }
