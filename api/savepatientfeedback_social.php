...
if (count($data) > 1) {
    ...
    //if ($res->num_rows == 0) {
    $query = 'INSERT INTO `bf_patients` (`guid`, `name`, `patient_id`, `mobile`, `email`,`admited_date`,`ward`,`bed_no`) VALUES ("' . time() . '","' . $data['name'] . '","' . $data['patientid'] . '","' . $data['contactnumber'] . '","' . $data['email'] . '","' . date('Y-m-d H:i:s') . '","' . $wardd->title . '","' . $bed . '")';
    ...
    $query = 'SELECT * FROM  `bf_feedback_social` WHERE  patientid="' . $patinet_id . '"';
    $res = mysqli_query($con, $query);
    ...
    $query = 'SELECT * FROM  `setup_social` WHERE 1';
    ...
    $query = 'INSERT INTO `bf_feedback_social`(`datetime`,`datet`,`remarks`, `nurseid`, `patientid`, `dataset`, `source`,`ward`,`bed_no`,`pid`) VALUES ("' . date('Y-m-d H:i:s') . '","' . $today . '","' . $data['remarks'] . '","' . $_GET['administratorId'] . '","' . $patinet_id . '","' . mysqli_real_escape_string($con, json_encode($data)) . '","' . $source . '","' . $wardd->title . '","' . $bed . '","' . $rid . '")';


    $result = mysqli_query($con, $query);
    $fid = mysqli_insert_id($con);

    $query = 'SELECT * FROM department WHERE type ="social"';
    $result = mysqli_query($con, $query);
    while ($r = mysqli_fetch_object($result)) {
        ...
        //             $query = 'INSERT INTO `tickets_social`(`created_by`, `departmentid`, `rating`, `anymessage`, `feedbackid`) VALUES ("' . $patinet_id . '","' . $r->dprt_id . '","1","' . $data['other'] . '","' . $fid . '")';
        //             mysqli_query($con, $query);
        //         }
        //     }
        // }
        ...
        foreach ($data['reason'] as $key => $value) {
            if ($r->slug == $key) {
                //var_dump($value);
                if ($value == true) {
                    $query = 'INSERT INTO `tickets_social`(`created_by`, `departmentid`, `rating`, `anymessage`, `feedbackid`,`ward`) VALUES ("' . $patinet_id . '","' . $r->dprt_id . '","' . $value . '","' . $data['comments'] . '","' . $fid . '","' . $wardd->title . '")';                    mysqli_query($con, $query);
                }
            }
        }
        ...
        //     if ($r->slug == $key) {
        //         if ($value == true) {
        //             $query = 'INSERT INTO `tickets_int`(`created_by`, `departmentid`, `rating`, `anymessage`, `feedbackid`) VALUES ("' . $patinet_id . '","' . $r->dprt_id . '","' . $value . '","' . $data['comments'] . '","' . $fid . '")';
        //             mysqli_query($con, $query);
