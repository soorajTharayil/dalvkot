...
$today = date('Y-m-d');
   ...
   $query = 'INSERT INTO `bf_feedback_22PSQ3c` (`name`,`email`,`patientid`,`mobile`,`sum_of_consultation_time`, `no_of_outpatients`, `avg_consultation_wait_time`, `bench_mark_time`, `data_analysis`,`corrective_action`,`preventive_action`, `dataset`,`datet`,`datetime`)
    VALUES ("' . $name . '","' . $email . '","' . $patientid . '","' . $contactnumber . '","' . $sum_of_consultation_time . '","' . $no_of_outpatients . '","' . $calculatedResult . '","' . $benchmark . '","' . $dataAnalysis . '","' . $correctiveAction . '","' . $preventiveAction . '","' . mysqli_real_escape_string($con, json_encode($data)) . '","' . $today . '","' . $datetime . '")';
