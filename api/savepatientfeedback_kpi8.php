...
$today = date('Y-m-d');
	...
	$no_surgery_procedure_followed =	$data['initial_assessment_hr'];
	$no_surgery_performed =	$data['total_admission'];
	...
	$query = 'INSERT INTO `bf_feedback_8PSQ3a` (`name`,`email`,`patientid`,`mobile`,`surgery_procedure_followed`, `no_surgery_performed`, `percentage_of_surgery_adherence`, `bench_mark_time`, `data_analysis`,`corrective_action`,`preventive_action`, `dataset`,`datet`,`datetime`) 
	VALUES ("' . $name . '","' . $email . '","' . $patientid . '","' . $contactnumber . '","' . $no_surgery_procedure_followed . '","' . $no_surgery_performed . '","' . $calculatedResult . '","' . $benchmark . '","' . $dataAnalysis . '","' . $correctiveAction . '","' . $preventiveAction . '","' . mysqli_real_escape_string($con, json_encode($data)) . '","' . $today . '","' . $datetime . '")';
