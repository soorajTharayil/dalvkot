...
$sql = 'SELECT * FROM `bf_departmentop` WHERE guid="'.$ward.'"' ;
$result = mysqli_query($con,$sql);
$datas = mysqli_fetch_object($result);
$i=0;
	//$data	 = array();
	$sql = 'SELECT * FROM `bf_opatients` WHERE ward="'.$datas->title.'" AND (discharged_date="0" OR discharged_date="") order by id desc' ;
	$result = mysqli_query($con,$sql);
	...
	//$data['paitent'][$i]['desies'] = 'Pain';
			...
			$sqls = 'SELECT * FROM `bf_outfeedback` WHERE patientid = "'.$row->patient_id.'"' ;
