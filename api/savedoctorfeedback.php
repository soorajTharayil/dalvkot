...
if (count($data) > 1) {
	...
	$query = 'SELECT * FROM  `bf_feedback_doctors` WHERE  patientid="' . $patinet_id . '"';
	...
	$query = 'SELECT * FROM  `setup_doctor` WHERE 1';
	...
	$query = 'INSERT INTO `bf_feedback_doctors`(`datetime`,`datet`,`remarks`, `nurseid`, `patientid`, `dataset`, `source`,`ward`,`bed_no`,`pid`,`consultant_cat`,`consultant_doc`) VALUES ("' . date('Y-m-d H:i:s') . '","' . $today . '","' . $data['remarks'] . '","' . $_GET['administratorId'] . '","' . $patinet_id . '","' . mysqli_real_escape_string($con, json_encode($data)) . '","' . $source . '","' . $wardd->title . '","' . $bed . '","' . $rid . '","' . $data['consultant_cat'] . '","' . $data['consultant_doc'] . '")';
	...
	$query = 'SELECT * FROM department WHERE type="doctor"';
		...
		foreach ($data as $key => $value) {
			...
			if ($r->slug == $key) {
				...
				if ($value == 2 || $value == 1) {
					...
					$query = 'INSERT INTO `tickets_doctor`(`created_by`, `departmentid`, `rating`, `anymessage`, `feedbackid`,`ward`) VALUES ("' . $patinet_id . '","' . $r->dprt_id . '","' . $value . '","' . $data['comments'] . '","' . $fid . '","' . $wardd->title . '")';
					mysqli_query($con, $query);
				}
			}
		}
