...
if (count($data) > 1) {
	...
	$no_of_hygiene_action =	$data['initial_assessment_hr'];
	$no_of_hygiene_oppurtunities =	$data['total_admission'];
   ...
   $query = 'INSERT INTO `bf_feedback_17PSQ3b` (`name`,`email`,`patientid`,`mobile`,`no_of_hygiene_action`, `no_of_hygiene_oppurtunities`, `hygiene_compliance_rate`, `bench_mark_time`, `data_analysis`,`corrective_action`,`preventive_action`, `dataset`,`datet`,`datetime`) 
   VALUES ("' . $name . '","' . $email . '","' . $patientid . '","' . $contactnumber . '","' . $no_of_hygiene_action . '","' . $no_of_hygiene_oppurtunities . '","' . $calculatedResult . '","' . $benchmark . '","' . $dataAnalysis . '","' . $correctiveAction . '","' . $preventiveAction . '","' . mysqli_real_escape_string($con, json_encode($data)) . '","' . $datet . '","' . $datetime . '")';
