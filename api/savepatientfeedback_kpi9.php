...
if (count($data) > 1) {
	...
	$no_of_transfusion_reactions =	$data['initial_assessment_hr'];
	$no_of_units_transfused =	$data['total_admission'];
	...
	$query = 'INSERT INTO `bf_feedback_9PSQ3a` (`name`,`email`,`patientid`,`mobile`,`no_transfusion_reactions`, `no_units_transfused`, `percentage_of_transfusion_reactions`, `bench_mark_time`, `data_analysis`,`corrective_action`,`preventive_action`, `dataset`,`datet`,`datetime`) 
	VALUES ("' . $name . '","' . $email . '","' . $patientid . '","' . $contactnumber . '","' . $no_of_transfusion_reactions . '","' . $no_of_units_transfused . '","' . $calculatedResult . '","' . $benchmark . '","' . $dataAnalysis . '","' . $correctiveAction . '","' . $preventiveAction . '","' . mysqli_real_escape_string($con, json_encode($data)) . '","' . $datet . '","' . $datetime . '")';
