...
$today = date('Y-m-d');
	...
	$no_of_blood_stream_infection =	$data['initial_assessment_hr'];
	$no_central_line_days =	$data['total_admission'];
   ...
   $query = 'INSERT INTO `bf_feedback_15PSQ3b` (`name`,`email`,`patientid`,`mobile`,`no_of_blood_stream_infection`, `no_central_line_days`, `blood_stream_infection_rate`, `bench_mark_time`, `data_analysis`,`corrective_action`,`preventive_action`, `dataset`,`datet`,`datetime`) 
   VALUES ("' . $name . '","' . $email . '","' . $patientid . '","' . $contactnumber . '","' . $no_of_blood_stream_infection . '","' . $no_central_line_days . '","' . $calculatedResult . '","' . $benchmark . '","' . $dataAnalysis . '","' . $correctiveAction . '","' . $preventiveAction . '","' . mysqli_real_escape_string($con, json_encode($data)) . '","' . $today . '","' . $datetime . '")';
