...
if (count($data) > 1) {
    ...
    $query = 'SELECT * FROM  `bf_feedback_int` WHERE  patientid="' . $patinet_id . '"';
    $res = mysqli_query($con, $query);
    ...
    $query = 'INSERT INTO `bf_feedback_int`(`datetime`,`datet`,`remarks`, `nurseid`, `patientid`, `dataset`, `source`,`ward`,`bed_no`,`pid`) VALUES ("' . date('Y-m-d H:i:s') . '","' . $today . '","' . $data['remarks'] . '","' . $_GET['administratorId'] . '","' . $patinet_id . '","' . mysqli_real_escape_string($con, json_encode($data)) . '","' . $source . '","' . $wardd->title . '","' . $bed . '","' . $rid . '")';


    $result = mysqli_query($con, $query);
    $fid = mysqli_insert_id($con);

    $query = 'SELECT * FROM department WHERE type ="interim"';
    $result = mysqli_query($con, $query);
    while ($r = mysqli_fetch_object($result)) {
        ...
        //             $query = 'INSERT INTO `tickets_int`(`created_by`, `departmentid`, `rating`, `anymessage`, `feedbackid`) VALUES ("' . $patinet_id . '","' . $r->dprt_id . '","1","' . $data['other'] . '","' . $fid . '")';
        //             mysqli_query($con, $query);
        //         }
        //     }
        // }
        ...
        foreach ($data['reason'] as $key => $value) {
            if ($r->slug == $key) {
                //var_dump($value);
                if ($value == true) {
                    $query = 'INSERT INTO `tickets_int`(`created_by`, `departmentid`, `rating`, `anymessage`, `feedbackid`,`ward`) VALUES ("' . $patinet_id . '","' . $r->dprt_id . '","' . $value . '","' . $data['comments'] . '","' . $fid . '","' . $wardd->title . '")';                    mysqli_query($con, $query);
                }
            }
        }
