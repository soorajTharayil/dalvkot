...
$today = date('Y-m-d');
	...
	$no_of_variations =	$data['initial_assessment_hr'];
	$no_of_variations_opportunities =	$data['total_admission'];
   ...
   $query = 'INSERT INTO `bf_feedback_27PSQ4d` (`name`,`email`,`patientid`,`mobile`,`no_of_variations`, `no_of_variations_opportunities`, `percentage_of_variations`, `bench_mark_time`, `data_analysis`,`corrective_action`,`preventive_action`, `dataset`,`datet`,`datetime`) 
   VALUES ("' . $name . '","' . $email . '","' . $patientid . '","' . $contactnumber . '","' . $no_of_variations . '","' . $no_of_variations_opportunities . '","' . $calculatedResult . '","' . $benchmark . '","' . $dataAnalysis . '","' . $correctiveAction . '","' . $preventiveAction . '","' . mysqli_real_escape_string($con, json_encode($data)) . '","' . $today . '","' . $datetime . '")';
