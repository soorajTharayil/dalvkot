...
$today = date('Y-m-d');
	...
	$no_of_prescriptions_in_capitals =	$data['initial_assessment_hr'];
	$no_of_prescriptions =	$data['total_admission'];
   ...
   $query = 'INSERT INTO `bf_feedback_32PSQ3d` (`name`,`email`,`patientid`,`mobile`,`no_of_prescriptions_in_capitals`, `no_of_prescriptions`, `compliance_rate`, `bench_mark_time`, `data_analysis`,`corrective_action`,`preventive_action`, `dataset`,`datet`,`datetime`) 
   VALUES ("' . $name . '","' . $email . '","' . $patientid . '","' . $contactnumber . '","' . $no_of_prescriptions_in_capitals . '","' . $no_of_prescriptions . '","' . $calculatedResult . '","' . $benchmark . '","' . $dataAnalysis . '","' . $correctiveAction . '","' . $preventiveAction . '","' . mysqli_real_escape_string($con, json_encode($data)) . '","' . $today . '","' . $datetime . '")';
