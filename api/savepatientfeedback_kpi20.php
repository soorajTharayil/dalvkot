...
if (count($data) > 1) {
	...
	$sum_of_time_taken =	$data['initial_assessment_total'];
	$no_of_blood_crossmatched =	$data['total_admission'];
   ...
   $query = 'INSERT INTO `bf_feedback_20PSQ3c` (`name`,`email`,`patientid`,`mobile`,`sum_of_time_taken`, `no_of_blood_crossmatched`, `average_turn_around_time`, `bench_mark_time`, `data_analysis`,`corrective_action`,`preventive_action`, `dataset`,`datet`,`datetime`) 
   VALUES ("' . $name . '","' . $email . '","' . $patientid . '","' . $contactnumber . '","' . $sum_of_time_taken . '","' . $no_of_blood_crossmatched . '","' . $calculatedResult . '","' . $benchmark . '","' . $dataAnalysis . '","' . $correctiveAction . '","' . $preventiveAction . '","' . mysqli_real_escape_string($con, json_encode($data)) . '","' . $datet . '","' . $datetime . '")';
