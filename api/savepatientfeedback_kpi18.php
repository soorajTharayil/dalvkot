...
$today = date('Y-m-d');
	...
	$no_of_receive_prophylactic =	$data['initial_assessment_hr'];
	$no_of_underwent_surgery =	$data['total_admission'];
   ...
   $query = 'INSERT INTO `bf_feedback_18PSQ3b` (`name`,`email`,`patientid`,`mobile`,`no_of_receive_prophylactic`, `no_of_underwent_surgery`, `prophylactic_percentage`, `bench_mark_time`, `data_analysis`,`corrective_action`,`preventive_action`, `dataset`,`datet`,`datetime`) 
   VALUES ("' . $name . '","' . $email . '","' . $patientid . '","' . $contactnumber . '","' . $no_of_receive_prophylactic . '","' . $no_of_underwent_surgery . '","' . $calculatedResult . '","' . $benchmark . '","' . $dataAnalysis . '","' . $correctiveAction . '","' . $preventiveAction . '","' . mysqli_real_escape_string($con, json_encode($data)) . '","' . $today . '","' . $datetime . '")';
