...
$today = date('Y-m-d');
	...
	$no_of_pneumonia =	$data['initial_assessment_hr'];
	$no_ventilator_days =	$data['total_admission'];
   ...
   $query = 'INSERT INTO `bf_feedback_14PSQ3b` (`name`,`email`,`patientid`,`mobile`,`no_of_pneumonia`, `no_ventilator_days`, `ventilator_pneumonia_rate`, `bench_mark_time`, `data_analysis`,`corrective_action`,`preventive_action`, `dataset`,`datet`,`datetime`) 
   VALUES ("' . $name . '","' . $email . '","' . $patientid . '","' . $contactnumber . '","' . $no_of_pneumonia . '","' . $no_ventilator_days . '","' . $calculatedResult . '","' . $benchmark . '","' . $dataAnalysis . '","' . $correctiveAction . '","' . $preventiveAction . '","' . mysqli_real_escape_string($con, json_encode($data)) . '","' . $today . '","' . $datetime . '")';
