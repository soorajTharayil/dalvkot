...
$path = '../uploads/' . $data['setting_data']->ip_qr_code_image;
$type = pathinfo($path, PATHINFO_EXTENSION);
$datap = file_get_contents($path);
$data['setting_data']->ip_qr_code_image = 'data:image/' . $type . ';base64,' . base64_encode($datap);
$baseurl = $config_set['BASE_URL'];

$path = '../uploads/' . $data['setting_data']->op_qr_code_image;
$type = pathinfo($path, PATHINFO_EXTENSION);
$datap = file_get_contents($path);
$data['setting_data']->op_qr_code_image = 'data:image/' . $type . ';base64,' . base64_encode($datap);
$baseurl = $config_set['BASE_URL'];
