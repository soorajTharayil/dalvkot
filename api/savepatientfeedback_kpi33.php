...
$today = date('Y-m-d');
	...
	$no_of_readmissions_icu =	$data['initial_assessment_hr'];
	$no_of_admissions =	$data['total_admission'];
	...
	$query = 'INSERT INTO `bf_feedback_PSQ3a` (`name`, `email`, `patientid`, `mobile`, `no_of_readmissions_icu`, `no_of_admissions`, `readmission_rate`, `bench_mark_time`, `data_analysis`, `corrective_action`, `preventive_action`, `dataset`, `datet`, `datetime`) 
	VALUES ("' . $name . '", "' . $email . '", "' . $patientid . '", "' . $contactnumber . '", "' . $no_of_readmissions_icu . '", "' . $no_of_admissions . '", "' . $calculatedResult . '", "' . $benchmark . '", "' . $dataAnalysis . '", "' . $correctiveAction . '", "' . $preventiveAction . '", "' . mysqli_real_escape_string($con, json_encode($data)) . '", "' . $today . '", "' . $datetime . '")';
