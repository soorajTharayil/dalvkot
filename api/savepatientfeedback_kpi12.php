...
if (count($data) > 1) {
	...
	$no_of_pressure_ulcer =	$data['initial_assessment_hr'];
	$no_of_patient_days =	$data['total_admission'];
	...
	$query = 'INSERT INTO `bf_feedback_12PSQ3a` (`name`,`email`,`patientid`,`mobile`,`no_of_pressure_ulcer`, `no_of_patient_days`, `incidence_of_pressure_ulcer`, `bench_mark_time`, `data_analysis`,`corrective_action`,`preventive_action`, `dataset`,`datetime`,`datet`) 
	VALUES ("' . $name . '","' . $email . '","' . $patientid . '","' . $contactnumber . '","' . $no_of_pressure_ulcer . '","' . $no_of_patient_days . '","' . $calculatedResult . '","' . $benchmark . '","' . $dataAnalysis . '","' . $correctiveAction . '","' . $preventiveAction . '","' . mysqli_real_escape_string($con, json_encode($data)) . '","' . $datetime . '","' . $datet . '")';
