...
if (count($data) > 1) {
	...
	$no_of_surgical_infection =	$data['initial_assessment_hr'];
	$no_of_surgery_perform =	$data['total_admission'];
   ...
   $query = 'INSERT INTO `bf_feedback_16PSQ3b` (`name`,`email`,`patientid`,`mobile`,`no_of_surgical_infection`, `no_of_surgery_perform`, `surgical_infection_rate`, `bench_mark_time`, `data_analysis`,`corrective_action`,`preventive_action`, `dataset`,`datet`,`datetime`) 
   VALUES ("' . $name . '","' . $email . '","' . $patientid . '","' . $contactnumber . '","' . $no_of_surgical_infection . '","' . $no_of_surgery_perform . '","' . $calculatedResult . '","' . $benchmark . '","' . $dataAnalysis . '","' . $correctiveAction . '","' . $preventiveAction . '","' . mysqli_real_escape_string($con, json_encode($data)) . '","' . $datet . '","' . $datetime . '")';
