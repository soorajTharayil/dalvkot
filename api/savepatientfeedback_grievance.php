...
if (count($data) > 1) {
    ...
    // $query = 'SELECT * FROM  `bf_employees_grievance` WHERE  mobile="' . $data['contactnumber'] . '"';
    // $res = mysqli_query($con, $query);
    //if ($res->num_rows == 0) {
    $query = 'INSERT INTO `bf_employees_grievance` (`guid`, `name`, `patient_id`, `mobile`, `email`,`admited_date`,`ward`,`bed_no`) VALUES ("' . time() . '","' . $data['name'] . '","' . $data['patientid'] . '","' . $data['contactnumber'] . '","' . $data['email'] . '","' . date('Y-m-d H:i:s') . '","' . $wardd->title . '","' . $bed . '")';
    $result = mysqli_query($con, $query);
    $rid = mysqli_insert_id($con);
    // } else {
    //     $patient_detail = mysqli_fetch_object($res);

    //     //$query = 'UPDATE `bf_employees_grievance` SET discharged_date= "' . date('Y-m-d H:i:s') . '"   WHERE  patient_id="' . $data['patientid'] . '"';
    //     //mysqli_query($con, $query);
    //     $rid = $patient_detail->id; 
    // }
    $patinet_id = $data['patientid'];
    $query = 'SELECT * FROM  `bf_feedback_grievance` WHERE  patientid="' . $patinet_id . '"';
    $res = mysqli_query($con, $query);
    ...
    $query = 'SELECT * FROM  `setup_grievance` WHERE 1';
    ...
    $query = 'INSERT INTO `bf_feedback_grievance`(`datetime`,`datet`,`remarks`, `nurseid`, `patientid`, `dataset`, `source`,`ward`,`bed_no`,`pid`) VALUES ("' . date('Y-m-d H:i:s') . '","' . $today . '","' . $data['remarks'] . '","' . $_GET['administratorId'] . '","' . $patinet_id . '","' . mysqli_real_escape_string($con, json_encode($data)) . '","' . $source . '","' . $wardd->title . '","' . $bed . '","' . $rid . '")';


    $result = mysqli_query($con, $query);
    $fid = mysqli_insert_id($con);

    $query = 'SELECT * FROM department WHERE type ="grievance"';
    $result = mysqli_query($con, $query);
    while ($r = mysqli_fetch_object($result)) {
        ...
        foreach ($data['reason'] as $key => $value) {
            if ($r->slug == $key) {
                //var_dump($value);
                if ($value == true) {
                    $query = 'INSERT INTO `tickets_grievance` (`created_by`, `departmentid`, `rating`, `anymessage`, `feedbackid`,`ward`) VALUES ("' . $patinet_id . '","' . $r->dprt_id . '","' . $value . '","' . $data['comments'] . '","' . $fid . '","' . $wardd->title . '")';                    mysqli_query($con, $query);
                }
            }
        }
    }
