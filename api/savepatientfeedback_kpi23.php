...
if (count($data) > 1) {
	...
	$sum_of_reporting_time =	$data['initial_assessment_total'];
	$no_of_patients_in_diagnostics =	$data['total_admission'];
   ...
   $query = 'INSERT INTO `bf_feedback_23PSQ4c` (`name`,`email`,`patientid`,`mobile`,`datetime`,`datet`,`sum_of_reporting_time`, `no_of_patients_in_diagnostics`, `diagnostics_wait_time`, `bench_mark_time`, `data_analysis`,`corrective_action`,`preventive_action`, `dataset`) 
   VALUES ("' . $name . '","' . $email . '","' . $patientid . '","' . $contactnumber . '","' . $datetime . '","' . $datet . '","' . $sum_of_reporting_time . '","' . $no_of_patients_in_diagnostics . '","' . $calculatedResult . '","' . $benchmark . '","' . $dataAnalysis . '","' . $correctiveAction . '","' . $preventiveAction . '","' . mysqli_real_escape_string($con, json_encode($data)) . '")';
