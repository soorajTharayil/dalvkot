...
if (count($data) > 1) {
	...
	$no_of_patients_fall =	$data['initial_assessment_hr'];
   ...
   $query = 'INSERT INTO `bf_feedback_28PSQ4d` (`name`,`email`,`patientid`,`mobile`,`no_of_patients_fall`, `no_patient_days`, `patient_fall_rate`, `bench_mark_time`, `data_analysis`,`corrective_action`,`preventive_action`, `dataset`,`datet`,`datetime`) 
   VALUES ("' . $name . '","' . $email . '","' . $patientid . '","' . $contactnumber . '","' . $no_of_patients_fall . '","' . $no_patient_days . '","' . $calculatedResult . '","' . $benchmark . '","' . $dataAnalysis . '","' . $correctiveAction . '","' . $preventiveAction . '","' . mysqli_real_escape_string($con, json_encode($data)) . '","' . $datet . '","' . $datetime . '")';
