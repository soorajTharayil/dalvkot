...
if (count($data) > 1) {
	...
	$no_of_reschedule =	$data['initial_assessment_hr'];
	$no_of_surgery_planned =	$data['total_admission'];
   ...
   $query = 'INSERT INTO `bf_feedback_19PSQ3c` (`name`,`email`,`patientid`,`mobile`,`no_of_reschedule`, `no_of_surgery_planned`, `percentage_of_reschedule`, `bench_mark_time`, `data_analysis`,`corrective_action`,`preventive_action`, `dataset`,`datet`,`datetime`) 
   VALUES ("' . $name . '","' . $email . '","' . $patientid . '","' . $contactnumber . '","' . $no_of_reschedule . '","' . $no_of_surgery_planned . '","' . $calculatedResult . '","' . $benchmark . '","' . $dataAnalysis . '","' . $correctiveAction . '","' . $preventiveAction . '","' . mysqli_real_escape_string($con, json_encode($data)) . '","' . $datet . '","' . $datetime . '")';
