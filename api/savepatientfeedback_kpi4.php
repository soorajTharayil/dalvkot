...
if (count($data) > 1) {
	...
	$no_medication_errors =	$data['initial_assessment_hr'];
	$no_opportunity_errors =	$data['total_admission'];
	...
	$contactnumber =	$data['contactnumber'];
	 ...
	 $query = 'INSERT INTO `bf_feedback_4PSQ3a` (`name`, `email`, `patientid`, `mobile`, `datetime`, `no_medication_errors`, `no_opportunity_errors`, `percentage_medication_errors`, `bench_mark_time`, `data_analysis`, `corrective_action`, `preventive_action`, `dataset`, `datet`) 
	 VALUES ("' . $name . '", "' . $email . '", "' . $patientid . '", "' . $contactnumber . '", "' . date('Y-m-d H:i:s') . '", "' . $no_medication_errors . '", "' . $no_opportunity_errors . '", "' . $calculatedResult . '", "' . $benchmark . '", "' . $dataAnalysis . '", "' . $correctiveAction . '", "' . $preventiveAction . '", "' . mysqli_real_escape_string($con, json_encode($data)) . '", "' . $datet . '")';
