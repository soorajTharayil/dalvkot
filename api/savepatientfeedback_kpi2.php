...
if (count($data) > 1) {
	...
	VALUES ("' . $name . '", "' . $email . '", "' . $patientid . '", "' . $contactnumber . '", "' . $no_reporting_error_total . '", "' . $test_performed . '", "' . $calculatedResult . '", "' . $benchmark . '", "' . $dataAnalysis . '", "' . $correctiveAction . '", "' . $preventiveAction . '", "' . mysqli_real_escape_string($con, json_encode($data)) . '", "' . $datet . '", "' . $datetime . '")';
