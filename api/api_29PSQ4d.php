...
$errorsCount = 0; // Initialize the count variable for incidents of type 'Near miss'
...
// echo '2';
    ...
    while ($row = mysqli_fetch_object($result)) {
        ...
        $incidentMonth = date('m', strtotime($row->datet));

        // Check if the incident belongs to the current month and the category is "Near miss"
        if ($incident_type === 'Near miss' && $incidentMonth === $selectedMonth) {
            $errorsCount++;
        }

        $data['incident'][$j]['datetime'] = $row->datet;
