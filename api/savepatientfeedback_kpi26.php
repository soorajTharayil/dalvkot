...
$today = date('Y-m-d');
	...
	$emergency_drugs_stock =	$data['initial_assessment_hr'];
	$no_listed_emergency_drugs =	$data['total_admission'];
   ...
   $query = 'INSERT INTO `bf_feedback_26PSQ4c` (`name`,`email`,`patientid`,`mobile`,`emergency_drugs_stock`, `no_listed_emergency_drugs`, `percentage_stock_out_rate`, `bench_mark_time`, `data_analysis`,`corrective_action`,`preventive_action`, `dataset`,`datet`,`datetime`) 
   VALUES ("' . $name . '","' . $email . '","' . $patientid . '","' . $contactnumber . '","' . $emergency_drugs_stock . '","' . $no_listed_emergency_drugs . '","' . $calculatedResult . '","' . $benchmark . '","' . $dataAnalysis . '","' . $correctiveAction . '","' . $preventiveAction . '","' . mysqli_real_escape_string($con, json_encode($data)) . '","' . $today . '","' . $datetime . '")';
