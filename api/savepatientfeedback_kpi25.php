...
$today = date('Y-m-d');
	...
	$no_of_incomplete_medical_records =	$data['initial_assessment_hr'];
	$no_of_discharge =	$data['total_admission'];
   ...
   $query = 'INSERT INTO `bf_feedback_25PSQ4c` (`name`,`email`,`patientid`,`mobile`,`no_of_incomplete_medical_records`, `no_of_discharge`, `percentage_of_medical_records`, `bench_mark_time`, `data_analysis`,`corrective_action`,`preventive_action`, `dataset`,`datet`,`datetime`) 
   VALUES ("' . $name . '","' . $email . '","' . $patientid . '","' . $contactnumber . '","' . $no_of_incomplete_medical_records . '","' . $no_of_discharge . '","' . $calculatedResult . '","' . $benchmark . '","' . $dataAnalysis . '","' . $correctiveAction . '","' . $preventiveAction . '","' . mysqli_real_escape_string($con, json_encode($data)) . '","' . $today . '","' . $datetime . '")';
