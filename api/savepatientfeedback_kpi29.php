...
if (count($data) > 1) {
	...
	$no_of_near_misses =	$data['initial_assessment_hr'];
	$no_of_incident_reported =	$data['total_admission'];
   ...
   $query = 'INSERT INTO `bf_feedback_29PSQ4d` (`name`,`email`,`patientid`,`mobile`,`no_of_near_misses`, `no_of_incident_reported`, `percentage_of_near_misses`, `bench_mark_time`, `data_analysis`,`corrective_action`,`preventive_action`, `dataset`,`datet`,`datetime`) 
   VALUES ("' . $name . '","' . $email . '","' . $patientid . '","' . $contactnumber . '","' . $no_of_near_misses . '","' . $no_of_incident_reported . '","' . $calculatedResult . '","' . $benchmark . '","' . $dataAnalysis . '","' . $correctiveAction . '","' . $preventiveAction . '","' . mysqli_real_escape_string($con, json_encode($data)) . '","' . $datet . '","' . $datetime . '")';
