...
$valid_token = 'b2a5e7c9d3f1428c8f57a3e91a6f4f123';
...
if (!$id) {
    $check_patient_id = $conn->prepare("SELECT * FROM patient_discharge WHERE patient_id = ?");
    ...
    $check_mobile = $conn->prepare("SELECT * FROM patient_discharge WHERE mobile = ?");
    ...
    $check_both = $conn->prepare("SELECT * FROM patient_discharge WHERE patient_id = ? AND mobile = ?");
    if ($check_both === false) {
        die(json_encode(['status' => false, 'message' => 'Prepare failed: ' . $conn->error]));
    }
...
if (!$id) {
    // Insert new patient
    $stmt = $conn->prepare("INSERT INTO patient_discharge (name, hospital_id, email, mobile, patient_id, gender, age, admitedfor, ward, consultant, bed_no, admited_date, discharged_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    if ($stmt === false) {
        die(json_encode(['status' => false, 'message' => 'Prepare failed: ' . $conn->error]));
    }
    ...
    $stmt = $conn->prepare("UPDATE patient_discharge SET name = ?, hospital_id = ?, email = ?, mobile = ?, patient_id = ?, gender = ?, age = ?, admitedfor = ?, ward = ?, consultant = ?, bed_no = ?, admited_date = ?, discharged_date = ? WHERE id = ?");
    if ($stmt === false) {
        die(json_encode(['status' => false, 'message' => 'Prepare failed: ' . $conn->error]));
    }
