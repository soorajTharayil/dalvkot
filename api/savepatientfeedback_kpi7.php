...
if (count($data) > 1) {
	...
	$no_unplanned_return_ot =	$data['initial_assessment_hr'];
	$no_patients_in_ot =	$data['total_admission'];
	...
	$query = 'INSERT INTO `bf_feedback_7PSQ3a` 
	(`name`, `email`, `patientid`, `mobile`, `no_unplanned_return_ot`, `no_patients_in_ot`, 
	`percentage_of_unplanned_return_ot`, `bench_mark_time`, `data_analysis`, `corrective_action`, 
	`preventive_action`, `dataset`, `datet`, `datetime`) 
	VALUES (
		"' . $name . '", 
		"' . $email . '", 
		"' . $patientid . '", 
		"' . $contactnumber . '", 
		"' . $no_unplanned_return_ot . '", 
		"' . $no_patients_in_ot . '", 
		"' . $calculatedResult . '", 
		"' . $benchmark . '", 
		"' . $dataAnalysis . '", 
		"' . $correctiveAction . '", 
		"' . $preventiveAction . '", 
		"' . mysqli_real_escape_string($con, json_encode($data)) . '", 
		"' . $datet . '", 
		"' . $datetime . '"
	)';
