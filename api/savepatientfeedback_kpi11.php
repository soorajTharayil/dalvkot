...
if (count($data) > 1) {
	...
	$return_to_emergency =	$data['initial_assessment_hr'];
	$no_of_patients_emergency =	$data['total_admission'];
   ...
   $query = 'INSERT INTO `bf_feedback_11PSQ3a` (`name`,`email`,`patientid`,`mobile`,`return_to_emergency`, `no_of_patients_emergency`, `percentage_of_return`, `bench_mark_time`, `data_analysis`,`corrective_action`,`preventive_action`, `dataset`,`datetime`,`datet`) 
   VALUES ("' . $name . '","' . $email . '","' . $patientid . '","' . $contactnumber . '","' . $return_to_emergency . '","' . $no_of_patients_emergency . '","' . $calculatedResult . '","' . $benchmark . '","' . $dataAnalysis . '","' . $correctiveAction . '","' . $preventiveAction . '","' . mysqli_real_escape_string($con, json_encode($data)) . '","' . $datetime . '","' . $datet . '")';
