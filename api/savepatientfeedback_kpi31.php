...
$today = date('Y-m-d');
	...
	$no_of_handovers_done =	$data['initial_assessment_hr'];
	$no_of_handover_opportunities =	$data['total_admission'];
   ...
   $query = 'INSERT INTO `bf_feedback_31PSQ3d` (`name`,`email`,`patientid`,`mobile`,`no_of_handovers_done`, `no_of_handover_opportunities`, `percentage_of_handovers`, `bench_mark_time`, `data_analysis`,`corrective_action`,`preventive_action`, `dataset`,`datet`,`datetime`) 
   VALUES ("' . $name . '","' . $email . '","' . $patientid . '","' . $contactnumber . '","' . $no_of_handovers_done . '","' . $no_of_handover_opportunities . '","' . $calculatedResult . '","' . $benchmark . '","' . $dataAnalysis . '","' . $correctiveAction . '","' . $preventiveAction . '","' . mysqli_real_escape_string($con, json_encode($data)) . '","' . $today . '","' . $datetime . '")';
