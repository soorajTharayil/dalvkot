...
$today = date('Y-m-d');
	...
	$no_of_parenteral_exposures =	$data['initial_assessment_hr'];
	$no_patient_days =	$data['total_admission'];
   ...
   $query = 'INSERT INTO `bf_feedback_30PSQ3d` (`name`,`email`,`patientid`,`mobile`,`no_of_parenteral_exposures`, `no_patient_days`, `needle_stick_injuries`, `bench_mark_time`, `data_analysis`,`corrective_action`,`preventive_action`, `dataset`,`datet`,`datetime`) 
   VALUES ("' . $name . '","' . $email . '","' . $patientid . '","' . $contactnumber . '","' . $no_of_parenteral_exposures . '","' . $no_patient_days . '","' . $calculatedResult . '","' . $benchmark . '","' . $dataAnalysis . '","' . $correctiveAction . '","' . $preventiveAction . '","' . mysqli_real_escape_string($con, json_encode($data)) . '","' . $today . '","' . $datetime . '")';
