...
if (count($data) > 1) {
   ...
   $query = 'INSERT INTO `bf_feedback_23dPSQ4c` (`name`,`email`,`patientid`,`mobile`,`sum_of_reporting_time`, `no_of_patients_in_diagnostics`, `ctscan_wait_time`, `bench_mark_time`, `data_analysis`,`corrective_action`,`preventive_action`, `dataset`,`datet`,`datetime`) 
   VALUES ("' . $name . '","' . $email . '","' . $patientid . '","' . $contactnumber . '","' . $sum_of_reporting_time . '","' . $no_of_patients_in_diagnostics . '","' . $calculatedResult . '","' . $benchmark . '","' . $dataAnalysis . '","' . $correctiveAction . '","' . $preventiveAction . '","' . mysqli_real_escape_string($con, json_encode($data)) . '","' . $datet . '","' . $datetime . '")';
