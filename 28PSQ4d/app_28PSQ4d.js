...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculatePatientFallRate = function () {
      ...
      var patientFalls = document.getElementById("formula_para1").value;

      // Get the total number of patient days
      var patientDays = document.getElementById("formula_para2").value;
      ...
      // Convert to integers
      patientFalls = parseInt(patientFalls);
      patientDays = parseInt(patientDays);
      ...
      if (patientFalls === 0 && patientDays === 0) {
        ...
        // Calculate the patient fall rate per 1000 patient days safely
        $scope.calculatedResult = Math.round(
          (patientFalls / (patientDays || 1)) * 1000
        );
      }
