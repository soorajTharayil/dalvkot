...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window) {
	...
	$scope.setupapplication = function () {
		...
		//alert(id);
		$http.get($rootScope.baseurl_main + '/api_18PSQ3b.php?patientid=' + id + '&month=' + selectedMonths + '&year=' + selectedYears, { timeout: 20000 }).then(function (responsedata) {
			$scope.feedback.initial_assessment_hr = responsedata.data.antibioticCount;
	...
	$scope.calculateProphylacticAntibioticsRate = function () {
		...
		var patientsWithProphylacticAntibiotics = parseInt(document.getElementById('formula_para1').value);

		// Get the number of patients who underwent surgeries in the OT
		var patientsUnderwentSurgeries = parseInt(document.getElementById('formula_para2').value);
		...
		if (patientsWithProphylacticAntibiotics === 0 && patientsUnderwentSurgeries === 0) {
			...
			$scope.calculatedResult = ((patientsWithProphylacticAntibiotics / patientsUnderwentSurgeries) * 100).toFixed(2);
		}

		// Store the result in the feedback object for further use
		$scope.feedback.calculatedResult = $scope.calculatedResult;

		console.log("Calculated Prophylactic Antibiotics Rate", $scope.calculatedResult);
	...
	$scope.savefeedback = function () {
		...
		if ($scope.feedback.initial_assessment_hr > $scope.feedback.total_admission) {
			alert("Number of patients received prophylactic antibiotics must be less than number of patients who underwent surgeries.");
			return;
		}

		// First check for duplicates
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_18PSQ3b')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/savepatientfeedback_kpi18.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
