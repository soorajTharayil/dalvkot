...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateUnplannedReturnToOTRate = function () {
      ...
      if (totalSurgeriesInOT > 0 && unplannedReturnsToOT > totalSurgeriesInOT) {
        alert(
      ...
      // Safe zero-handling for denominator
      var unplannedReturnToOTRate;
      if (totalSurgeriesInOT === 0) {
        unplannedReturnToOTRate = 0; // avoid division by zero
      } else {
        unplannedReturnToOTRate =
