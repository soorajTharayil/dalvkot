...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window) {
	...
	$scope.setupapplication = function () {
		...
		//alert(id);
		$http.get($rootScope.baseurl_main + '/api_PSQ3a.php?patientid=' + id + '&month=' + selectedMonths + '&year=' + selectedYears, { timeout: 20000 }).then(function (responsedata) {
	...
	$scope.calculateAdmissionRate = function () {
		...
		var noOfReadmission = parseInt(document.getElementById('formula_para1').value);
		...
		if (noOfReadmission > totalAdmissions) {
			alert("The number of readmission to ICU within 48 hours must be less than the total number of admissions.");
			return;
		}

		// Calculate the percentage of compliance rate to medication prescription in capitals
		$scope.calculatedResult = ((noOfReadmission / totalAdmissions) * 100).toFixed(2);
	...
	$scope.savefeedback = function () {
		...
		if ($scope.feedback.initial_assessment_hr > $scope.feedback.total_admission) {
			alert("No. of readmission to ICU within 48 hours must be less than the total number of admissions.");
			return;
		}

		// First check for duplicates
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_PSQ3a')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/savepatientfeedback_kpi33.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
