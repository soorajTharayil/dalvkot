...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateNursePatientRatio = function () {
      ...
      // Allow ZERO — only alert if EMPTY (NaN)
      if (isNaN(nursingStaff)) {
        alert("Enter the number of nursing staff.");
        return;
      }

      if (isNaN(occupiedBeds)) {
      ...
      // Case: BOTH ZERO → Return safe output
      if (nursingStaff === 0 && occupiedBeds === 0) {
        $scope.calculatedResult = "0:0";
        $scope.feedback.calculatedResult = "0:0";
        $scope.explanation = "Both nursing staff and occupied beds are zero.";
        return;
      }

      // Case: nursing staff = 0 but beds > 0 → infinite workload
      if (nursingStaff === 0 && occupiedBeds > 0) {
        $scope.calculatedResult = "0:" + occupiedBeds;
        $scope.feedback.calculatedResult = $scope.calculatedResult;
        $scope.explanation = "No nurses available for the occupied beds.";
        return;
      }

      // Case: beds = 0 but staff > 0 → nurses available but no patients
      if (occupiedBeds === 0 && nursingStaff > 0) {
        $scope.calculatedResult = nursingStaff + ":0";
        $scope.feedback.calculatedResult = $scope.calculatedResult;
        $scope.explanation = "Nurses available but no patients.";
        return;
      }
