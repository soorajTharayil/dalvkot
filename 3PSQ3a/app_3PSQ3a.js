...
app.controller('PatientFeedbackCtrl', function ($rootScope, $scope, $http, $location, $window) {
	...
	$scope.setupapplication = function () {
		...
		//alert(id);
		$http.get($rootScope.baseurl_main + '/api_3PSQ3a.php?patientid=' + id + '&month=' + selectedMonths + '&year=' + selectedYears, { timeout: 20000 }).then(function (responsedata) {
			$scope.feedback.initial_assessment_hr = responsedata.data.staff_adhering_to_safety_precautions;
			$scope.feedback.total_admission = responsedata.data.staff_audited_count;
	...
	$scope.calculateAdherenceRate = function () {
		...
		var adheringStaff = parseInt(document.getElementById('formula_para1').value);

		// Get the number of staff audited
		var auditedStaff = parseInt(document.getElementById('formula_para2').value);
		...
		if (adheringStaff > auditedStaff) {
			alert("The no. of staff adhering to safety precautions must be less than the number of staff audited.");
			return;
		}
	...
	$scope.savefeedback = function () {
		...
		$http.get($rootScope.baseurl_main + '/quality_duplication_submission.php?patient_id=' + $rootScope.patientid + '&month=' + $scope.selectedMonths + '&year=' + $scope.selectedYears + '&table=' + 'bf_feedback_3PSQ3a')
			.then(function (response) {
				if (response.data.status === "exists") {
					...
					$http.post($rootScope.baseurl_main + '/savepatientfeedback_kpi3.php?patient_id=' + $rootScope.patientid + '&administratorId=' + $rootScope.adminId + '&month=' + selectedMonths + '&year=' + selectedYears, $scope.feedback).then(function (responsedata) {
