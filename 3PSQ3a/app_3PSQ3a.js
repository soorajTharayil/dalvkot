...
app.controller(
  ...
  function ($rootScope, $scope, $http, $location, $window, $timeout) {
    ...
    $scope.calculateAdherenceRate = function () {
      ...
      // Validate inputs for staff adherence (allow 0)
      if (isNaN(adheringStaff) || adheringStaff < 0) {
        alert("Enter the number of staff adhering to safety precautions.");
        return;
      }

      // Validate auditedStaff (only check NaN, allow 0)
      if (isNaN(auditedStaff)) {
        alert("Enter the number of staff audited.");
        return;
      }

      // Skip comparison if auditedStaff is 0
      ...
      // Calculate adherence — handle denominator = 0 safely
      var adherencePercentage;
      if (auditedStaff === 0) {
        adherencePercentage = 0; // Avoid division by zero
      } else {
        adherencePercentage = (adheringStaff / auditedStaff) * 100;
      }
